...
def test_algorithm_quickly(algorithm_fn, num_tests=50):
    ...
    print(f"  Success rate: {sum(1 for r in results if r['similarity'] > 0.9)/len(results)*100:.1f}%")
    ...
    for steps in param_grid['max_episode_steps']:
        for bonus in param_grid['success_bonus']:
            for penalty in param_grid['step_penalty']:
                ...
                print(f"Steps={steps}, Bonus={bonus}, Penalty={penalty}: {avg_reward:.3f}")
...
- **[Advanced Patterns](advanced-patterns.md)**: Scale up with JAX transformations
- **[Basic Usage](basic-usage.md)**: Learn fundamental JaxARC patterns
- **[Configuration Guide](../configuration.md)**: Optimize configurations for your use case
