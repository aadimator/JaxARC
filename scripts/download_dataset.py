...
def _log_dataset_info(config: DatasetConfig, downloaded_path: Path) -> None:
    ...
    if "training_tasks" in structure and "evaluation_tasks" in structure:
        logger.info(
            f"  - {structure['training_tasks']} training tasks in {structure['training_dir']}"
        )
        logger.info(
            f"  - {structure['evaluation_tasks']} evaluation tasks in {structure['evaluation_dir']}"
        )
    elif "concept_groups" in structure:
        logger.info(
            f"  - {structure['concept_groups']} concept groups in {structure['data_dir']}"
        )
        logger.info(f"  - {structure['tasks_per_group']} tasks per concept group")
    elif "total_tasks" in structure:
        logger.info(
            f"  - {structure['total_tasks']} task files in {structure['data_dir']}"
        )
...
def _download_single_dataset(
    dataset_key: str, output_dir: Path | None = None, force: bool = False
) -> None:
...
def _download_all_datasets(output_dir: Path, force: bool) -> None:
    ...
    for config in DATASETS.values():
        ...
        if target_dir.exists() and not force:
            logger.info(
                f"{config.display_name} already exists, skipping (use --force to re-download)"
            )
            continue
