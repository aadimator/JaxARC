...
@jax.jit
def apply_within_bounds(grid: jnp.ndarray, selection: jnp.ndarray,
                       new_values: jnp.ndarray) -> jnp.ndarray:
...
def fill_color(state: ARCLEState, selection: jnp.ndarray, color: int) -> ARCLEState:
    ...
    new_grid = apply_within_bounds(state.grid, selection, color)
...
@jax.jit
def simple_flood_fill(grid: jnp.ndarray, selection: jnp.ndarray,
                     fill_color: int, max_iterations: int = 64) -> jnp.ndarray:
    ...
    def do_flood_fill():
        ...
        final_flood_mask = jax.lax.fori_loop(0, max_iterations, flood_step, initial_flood_mask)
...
@jax.jit
def flood_fill_color(state: ARCLEState, selection: jnp.ndarray, color: int) -> ARCLEState:
    """Flood fill from selected region with specified color."""
    new_grid = simple_flood_fill(state.grid, selection, color)
    return state.replace(grid=new_grid)
...
@jax.jit
def move_object(state: ARCLEState, selection: jnp.ndarray, direction: int) -> ARCLEState:
    """Move selected object in specified direction (0=up, 1=down, 2=left, 3=right)."""
    # Extract selected object
    object_pixels = jnp.where(selection, state.grid, 0)

    # Clear original positions
    cleared_grid = jnp.where(selection, 0, state.grid)

    # Move object based on direction
    def move_up(): return jnp.roll(object_pixels, -1, axis=0)
    def move_down(): return jnp.roll(object_pixels, 1, axis=0)
    def move_left(): return jnp.roll(object_pixels, -1, axis=1)
    def move_right(): return jnp.roll(object_pixels, 1, axis=1)
...
def rotate_object(state: ARCLEState, selection: jnp.ndarray, angle: int) -> ARCLEState:
    ...
    # Rotate object
    def rotate_90(): return jnp.rot90(object_pixels, k=1)
    def rotate_270(): return jnp.rot90(object_pixels, k=3)
...
def flip_object(state: ARCLEState, selection: jnp.ndarray, axis: int) -> ARCLEState:
    ...
    # Flip object
    def flip_horizontal(): return jnp.fliplr(object_pixels)
    def flip_vertical(): return jnp.flipud(object_pixels)
...
def copy_to_clipboard(state: ARCLEState, selection: jnp.ndarray) -> ARCLEState:
    ...
    new_clipboard = jnp.where(selection, state.grid, 0)
...
def paste_from_clipboard(state: ARCLEState, selection: jnp.ndarray) -> ARCLEState:
    ...
    new_grid = jnp.where(paste_mask, state.clipboard, state.grid)
    return state.replace(grid=new_grid)
...
def cut_to_clipboard(state: ARCLEState, selection: jnp.ndarray) -> ARCLEState:
    ...
    new_grid = jnp.where(selection, 0, state.grid)

    return state.replace(grid=new_grid, clipboard=new_clipboard)
...
def clear_grid(state: ARCLEState, selection: jnp.ndarray) -> ARCLEState:
    ...
    def clear_selection():
        return jnp.where(selection, 0, state.grid)

    def clear_all():
        return jnp.zeros_like(state.grid)
...
def copy_input_grid(state: ARCLEState, selection: jnp.ndarray) -> ARCLEState:
    ...
    return state.replace(grid=state.input_grid)
...
def execute_arcle_operation(state: ARCLEState, operation: jnp.ndarray) -> ARCLEState:
    ...
    def op_0(): return fill_color(state, selection, 0)
    def op_1(): return fill_color(state, selection, 1)
    def op_2(): return fill_color(state, selection, 2)
    def op_3(): return fill_color(state, selection, 3)
    def op_4(): return fill_color(state, selection, 4)
    def op_5(): return fill_color(state, selection, 5)
    def op_6(): return fill_color(state, selection, 6)
    def op_7(): return fill_color(state, selection, 7)
    def op_8(): return fill_color(state, selection, 8)
    def op_9(): return fill_color(state, selection, 9)

    def op_10(): return flood_fill_color(state, selection, 0)
    def op_11(): return flood_fill_color(state, selection, 1)
    def op_12(): return flood_fill_color(state, selection, 2)
    def op_13(): return flood_fill_color(state, selection, 3)
    def op_14(): return flood_fill_color(state, selection, 4)
    def op_15(): return flood_fill_color(state, selection, 5)
    def op_16(): return flood_fill_color(state, selection, 6)
    def op_17(): return flood_fill_color(state, selection, 7)
    def op_18(): return flood_fill_color(state, selection, 8)
    def op_19(): return flood_fill_color(state, selection, 9)

    def op_20(): return move_object(state, selection, 0)  # up
    def op_21(): return move_object(state, selection, 1)  # down
    def op_22(): return move_object(state, selection, 2)  # left
    def op_23(): return move_object(state, selection, 3)  # right

    def op_24(): return rotate_object(state, selection, 0)  # 90°
    def op_25(): return rotate_object(state, selection, 1)  # 270°

    def op_26(): return flip_object(state, selection, 0)  # horizontal
    def op_27(): return flip_object(state, selection, 1)  # vertical

    def op_28(): return copy_to_clipboard(state, selection)
    def op_29(): return paste_from_clipboard(state, selection)
    def op_30(): return cut_to_clipboard(state, selection)

    def op_31(): return clear_grid(state, selection)
    def op_32(): return copy_input_grid(state, selection)
    def op_33(): return resize_grid(state, selection)

    def op_34(): return submit_solution(state, selection)
    ...
    operations = [
        ...
        op_28, op_29, op_30, op_31, op_32, op_33, op_34
    ]
