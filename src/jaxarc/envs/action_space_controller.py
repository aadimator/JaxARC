...
class ActionSpaceController:
    ...
    def get_allowed_operations(
        ...
        if (
            hasattr(config, "allowed_operations")
            and config.allowed_operations is not None
        ):
        ...
        if (
            hasattr(config, "dynamic_action_filtering")
            and config.dynamic_action_filtering
        ):
    ...
    def validate_operation(
        ...
        if not is_basic_valid:
            ...
            if not (0 <= operation_id < NUM_OPERATIONS):
                return (
                    False,
                    f"Operation ID {operation_id} is out of range [0, {NUM_OPERATIONS - 1}]",
                )
            else:
        ...
        context_valid, context_msg = self._validate_operation_context_jax(
            operation_id, state, config
        )
    ...
    def validate_operation_jax(
        ...
        is_context_valid = self._validate_operation_context_jax_only(
            operation_id, state
        )
    ...
    def _is_operation_valid_jax(
        self, operation_id: jnp.ndarray, allowed_mask: OperationMask
    ) -> jnp.ndarray:
        ...
        is_allowed = jnp.where(
            in_range, allowed_mask[jnp.clip(operation_id, 0, NUM_OPERATIONS - 1)], False
        )
    ...
    def _get_context_dependent_mask(
        ...
        demo_switching_ops = jnp.array(
            [35, 36, 40]
        )  # NEXT_DEMO, PREV_DEMO, FIRST_UNSOLVED_DEMO
        ...
        test_switching_ops = jnp.array(
            [37, 38, 41]
        )  # NEXT_TEST, PREV_TEST, FIRST_UNSOLVED_TEST
    ...
    def _validate_operation_context_jax(
        ...
        if operation_id in [35, 36, 40]:  # Demo switching operations
            if not bool(state.is_training_mode()):
                return (
                    False,
                    f"Demo switching operation {operation_id} not allowed in test mode",
                )
            ...
            if available_demo_count <= 1:
                return (
                    False,
                    f"Demo switching operation {operation_id} requires multiple demo pairs",
                )
        ...
        # Test pair switching operations (37, 38, 41)
            ...
            if not bool(state.is_test_mode()):
                return (
                    False,
                    f"Test switching operation {operation_id} not allowed in training mode",
                )
            ...
            if available_test_count <= 1:
                return (
                    False,
                    f"Test switching operation {operation_id} requires multiple test pairs",
                )
    ...
    def _validate_operation_context_jax_only(
        self, operation_id: jnp.ndarray, state: ArcEnvState
    ) -> jnp.ndarray:
    ...
    def get_operation_availability_summary(
        ...
        config_info = {
            "dynamic_filtering_enabled": getattr(
                config, "dynamic_action_filtering", False
            ),
            "context_dependent_enabled": getattr(
                config, "context_dependent_operations", False
            ),
            "explicit_allowed_operations": getattr(config, "allowed_operations", None),
            "invalid_operation_policy": getattr(
                config, "invalid_operation_policy", "clip"
            ),
        }
        ...
        return {
            ...
            "allowed_operations": [
                i for i in range(NUM_OPERATIONS) if bool(allowed_mask[i])
            ],
            "blocked_operations": [
                i for i in range(NUM_OPERATIONS) if not bool(allowed_mask[i])
            ],
    ...
    def apply_operation_mask_jax(
        ...
        mask_expanded = jnp.broadcast_to(allowed_mask, action_logits.shape)

        # Apply mask: keep original values for allowed ops, mask_value for invalid ops
        masked_logits = jnp.where(mask_expanded, action_logits, mask_value)
    ...
    def _find_next_valid_operation_jax(
        self, operation_id: jnp.ndarray, allowed_mask: OperationMask, step: int
    ) -> jnp.ndarray:
        ...
        return jnp.where(
            has_valid, candidates[first_valid_idx], jnp.array(-1, dtype=jnp.int32)
        )
    ...
    def create_action_mask_for_agent(
        ...
        return {
            ...
            "context_info": {
                "episode_mode": "train" if bool(state.is_training_mode()) else "test",
                "step_count": int(state.step_count),
                "can_switch_demos": bool(
                    operation_mask[35]
                ),  # SWITCH_TO_NEXT_DEMO_PAIR
                "can_switch_tests": bool(
                    operation_mask[37]
                ),  # SWITCH_TO_NEXT_TEST_PAIR
