...
Examples:
    ...
    from jaxarc.envs.actions import create_mask_action
...
class ActionRecord:
    ...
    def validate_integrity(
        ...
        base_valid = jnp.where(self.valid, jnp.array(True), jnp.array(False))

        # Validate operation ID range
        operation_valid = (self.operation_id >= 0) & (
            self.operation_id < NUM_OPERATIONS
        )
    ...
    def validate_integrity_detailed(
        ...
        if not (0 <= int(self.operation_id) < NUM_OPERATIONS):
            return (
                False,
                f"Operation ID {int(self.operation_id)} is out of range [0, {NUM_OPERATIONS - 1}]",
            )
        ...
        # Validate selection data based on format
        expected_size = get_selection_data_size(
            selection_format, max_grid_height, max_grid_width
        )
        if self.selection_data.shape[0] != expected_size:
            return (
                False,
                f"Selection data size {self.selection_data.shape[0]} != expected {expected_size}",
            )
        ...
        if selection_format == "point":
            if expected_size >= 2:
                ...
                if not (0 <= row < max_grid_height):
                    return (
                        False,
                        f"Point row {row} is out of bounds [0, {max_grid_height})",
                    )
                if not (0 <= col < max_grid_width):
                    return (
                        False,
                        f"Point col {col} is out of bounds [0, {max_grid_width})",
                    )
            ...
            if expected_size >= 4:
                ...
                if not (0 <= r1 < max_grid_height and 0 <= r2 < max_grid_height):
                    return (
                        False,
                        f"Bbox rows [{r1}, {r2}] are out of bounds [0, {max_grid_height})",
                    )
                if not (0 <= c1 < max_grid_width and 0 <= c2 < max_grid_width):
                    return (
                        False,
                        f"Bbox cols [{c1}, {c2}] are out of bounds [0, {max_grid_width})",
                    )
                if not (r1 <= r2 and c1 <= c2):
                    return (
                        False,
                        f"Bbox coordinates not properly ordered: ({r1}, {c1}) to ({r2}, {c2})",
                    )
    ...
    def _validate_selection_data(
        self, selection_format: str, max_grid_height: int, max_grid_width: int
    ) -> Bool[Array, ""]:
        ...
        basic_valid = jnp.all(jnp.isfinite(self.selection_data)) & jnp.all(
    ...
    def get_selection_coordinates(self, selection_format: str) -> Array:
        ...
        if selection_format == "mask":
            ...
            return jnp.where(
                self.selection_data > 0.5, size=self.selection_data.shape[0]
            )[0]
    ...
    def create_from_action(
        ...
        Examples:
            ...
            from jaxarc.envs.actions import PointAction, MaskAction

            action = PointAction(
                operation=jnp.array(15, dtype=jnp.int32),
                row=jnp.array(5, dtype=jnp.int32),
                col=jnp.array(10, dtype=jnp.int32),
            )
        ...
        if hasattr(action, "operation"):
            ...
            raise ValueError(
                f"Unsupported action type: {type(action)}. Only structured actions are supported."
            )
    ...
    def _format_selection_data(
        ...
        if hasattr(action, "operation"):
            from .actions import BboxAction, MaskAction, PointAction
            ...
            if selection_format == "bbox":
                if isinstance(action, BboxAction):
                    return jnp.array(
                        [action.r1, action.c1, action.r2, action.c2], dtype=jnp.float32
                    )
...
def validate_action_record_array(
    ...
    def validate_single_record(record_array):
        ...
        return temp_record.validate_integrity(
            selection_format, max_grid_height, max_grid_width
        )
...
def filter_valid_records(
    ...
    """
    valid_mask = validate_action_record_array(
        records, selection_format, max_grid_height, max_grid_width
    )
...
def create_padded_record_array(
    ...
    target_record_size = get_action_record_fields(
        selection_format, max_grid_height, max_grid_width
    )
...
class ActionHistoryTracker:
    ...
    def add_action(
        ...
        Examples:
            ...
            from jaxarc.envs.actions import PointAction, BboxAction, MaskAction
            ...
            action = BboxAction(
                ...
                c1=jnp.array(2, dtype=jnp.int32),
                r2=jnp.array(8, dtype=jnp.int32),
        ...
        return update_multiple_fields(
            state, action_history=new_history, action_history_length=new_length
        )
    ...
    def get_history_summary(self, state: State) -> dict:
        ...
        if history_length > 0:
            ...
            summary.update(
                {
                    "unique_operations": len(jnp.unique(operations)),
                    "most_recent_operation": int(operations[-1])
                    if len(operations) > 0
                    else -1,
                }
            )
...
def add_action_to_state(
    ...
    Examples:
        ...
        from jaxarc.envs.actions import PointAction, BboxAction
