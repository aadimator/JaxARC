...
def validate_grid_initialization_config(config: GridInitializationConfig) -> list[str]:
    ...
    for weight_name, weight_value in weights:
        if not isinstance(weight_value, (int, float)):
            errors.append(f"{weight_name} must be a number, got {type(weight_value).__name__}")
            continue
...
def validate_generated_grid(
    ...
    if grid.shape != mask.shape:
        errors.append(
            f"Grid shape {grid.shape} doesn't match mask shape {mask.shape}"
        )
        return errors
...
def create_fallback_grid(task: JaxArcTask) -> Tuple[GridArray, MaskArray]:
    ...
    try:
        ...
        if hasattr(task, 'get_grid_shape'):
            ...
            grid_shape = (10, 10)
            logger.warning(f"Task has no get_grid_shape method, using default {grid_shape}")
...
def log_initialization_debug_info(
    ...
    debug_info = {
        ...
        "permutation_types": list(config.permutation_types) if config.permutation_types else [],
