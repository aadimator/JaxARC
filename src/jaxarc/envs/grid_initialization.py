...
def initialize_working_grids(
    ...
    Examples:
        ...
        grids, masks = initialize_working_grids(task, config, key, batch_size=32, initial_pair_idx=2)
        ```
...
def _select_batch_modes(
    ...
    weights = jnp.where(
        weight_sum > 1e-8,
        weights / weight_sum,
        jnp.array([0.25, 0.25, 0.25, 0.25], dtype=jnp.float32)  # Equal weights fallback
    )
...
def _init_demo_grid(task: JaxArcTask, key: PRNGKey, initial_pair_idx: int | None = None) -> Tuple[GridArray, MaskArray]:
    ...
    def use_demo_grid():
        ...
        if initial_pair_idx is not None:
            ...
            demo_idx = jnp.clip(initial_pair_idx, 0, jnp.maximum(task.num_train_pairs - 1, 0))
        else:
...
def _apply_grid_permutations(
    ...
    def apply_permutation():
        return jax.lax.switch(
            perm_choice,
            [
                lambda: jax.lax.cond(has_rotate, 
                                   lambda: _apply_rotation(grid, apply_key), 
                                   lambda: grid),
                lambda: jax.lax.cond(has_reflect, 
                                   lambda: _apply_reflection(grid, apply_key), 
                                   lambda: grid),
                lambda: jax.lax.cond(has_color_remap, 
...
def _generate_dense_pattern(
    ...
    padded_sparse = jnp.pad(sparse_grid.astype(jnp.float32), 1, mode='constant', constant_values=0)
