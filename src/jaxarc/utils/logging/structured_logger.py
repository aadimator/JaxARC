...
class StructuredLogger:
    ...
    def start_episode(
        ...
        if self.current_episode is not None:
            logger.warning(f"Starting new episode {episode_num} without ending previous episode {self.current_episode.episode_num}")
            self.end_episode()
    ...
    def end_episode(self, summary_visualization_path: Optional[str] = None) -> None:
        ...
        object.__setattr__(self.current_episode, 'summary_visualization_path', summary_visualization_path)
    ...
    def _save_episode_sync(self, episode: EpisodeLogEntry) -> None:
        ...
        # Generate filename
        timestamp_str = time.strftime("%Y%m%d_%H%M%S", time.localtime(episode.start_timestamp))
