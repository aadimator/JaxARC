...
class PerformanceMonitor:
    ...
    def __init__(self, config: PerformanceConfig):
        ...
        self.samples: Dict[str, deque] = defaultdict(lambda: deque(maxlen=config.max_samples))
    ...
    def measure_function_impact(self, func: F, function_name: Optional[str] = None) -> F:
    ...
    def _record_sample(
        ...
        """
        overhead_ratio = visualization_time / execution_time if execution_time > 0 else 0.0
    ...
    def _check_performance_alert(self, function_name: str, overhead_ratio: float) -> None:
    ...
    def _adapt_logging_level(self) -> None:
        ...
        if avg_overhead > self.config.alert_threshold * 2:
            ...
            logger.info(f"Reducing logging level to {self.logging_level:.2f} due to high overhead ({avg_overhead:.1%})")
            ...
            logger.info(f"Reducing logging level to {self.logging_level:.2f} due to moderate overhead ({avg_overhead:.1%})")
        elif avg_overhead < self.config.alert_threshold * 0.5 and self.logging_level < 1.0:
            # Low overhead - increase logging
            self.logging_level = min(1.0, self.logging_level * 1.2)
            logger.info(f"Increasing logging level to {self.logging_level:.2f} due to low overhead ({avg_overhead:.1%})")
        ...
        if self.function_stats:
            ...
            report['overall'] = {
                ...
                'functions_over_threshold': sum(
                    1 for overhead in all_overheads 
    ...
    def export_samples(self, function_name: str, max_samples: Optional[int] = None) -> List[Dict[str, Any]]:
