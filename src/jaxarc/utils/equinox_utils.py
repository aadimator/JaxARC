                ...
                try:
                    ...
                    if jnp.issubdtype(value.dtype, jnp.floating):
                        if jnp.any(jnp.isnan(value)) or jnp.any(jnp.isinf(value)):
                            validation_errors.append(f"{path}: contains NaN or infinite values")
                    
                    # Check for negative values where inappropriate
                    if 'step_count' in path.lower() or 'index' in path.lower():
                        if jnp.any(value < 0):
                            validation_errors.append(f"{path}: contains negative values")
...
def create_state_diff(old_state: T, new_state: T) -> Dict[str, Dict[str, Any]]:
    ...
    def compare_values(old_val: Any, new_val: Any) -> Dict[str, Any] | None:
        try:
            if hasattr(old_val, 'shape') and hasattr(new_val, 'shape'):
                ...
                elif not jnp.array_equal(old_val, new_val):
                    return {
                        'type': 'value_change',
                        'old': old_val,
                        'new': new_val,
                        'max_diff': jnp.max(jnp.abs(old_val - new_val)) if jnp.issubdtype(old_val.dtype, jnp.number) else None,
                    }
            else:
    ...
    if isinstance(old_state, eqx.Module) and isinstance(new_state, eqx.Module):
        if hasattr(old_state, '__annotations__') and hasattr(new_state, '__annotations__'):
            # Compare each field
    ...
    def print_info(path: str, value: Any) -> Any:
        if hasattr(value, 'shape'):
            ...
            if hasattr(value, 'dtype') and jnp.issubdtype(value.dtype, jnp.number):
                try:
                    print(f"    min={jnp.min(value):.4f}, max={jnp.max(value):.4f}, mean={jnp.mean(value):.4f}")
                except Exception:
                    pass
        else:
...
def check_jax_transformations(module: T, test_fn: Callable[[T], Any]) -> Dict[str, bool]:
    ...
    try:
        ...
        batch_module = jtu.tree_map(lambda x: jnp.expand_dims(x, 0) if hasattr(x, 'shape') else x, module)
    ...
    try:
        ...
        def scalar_test_fn(m):
            result = test_fn(m)
            return jnp.sum(jnp.array(0.0)) if not hasattr(result, 'shape') else jnp.sum(result)
