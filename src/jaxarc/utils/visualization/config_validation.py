...
class ConfigValidator:
    ...
    def _validate_async_logger_config(self, async_config: DictConfig) -> None:
        ...
        if hasattr(async_config, "queue_size"):
            if (
                not isinstance(async_config.queue_size, int)
                or async_config.queue_size <= 0
            ):
                self.errors.append(
                    ValidationError(
        ...
        if hasattr(async_config, "worker_threads"):
            if (
                not isinstance(async_config.worker_threads, int)
                or async_config.worker_threads <= 0
            ):
                self.errors.append(
                    ValidationError(
    ...
    def _cross_validate_config(self, config: DictConfig) -> List[ValidationError]:
        ...
        if (
            hasattr(config, "visualization")
            and hasattr(config, "storage")
            and hasattr(config.visualization, "debug_level")
            and hasattr(config.storage, "max_storage_gb")
        ):
        ...
        if (
            ...
            and hasattr(config, "visualization")
            and hasattr(config.wandb, "enabled")
            and config.wandb.enabled
            and hasattr(config.visualization, "debug_level")
            and config.visualization.debug_level == "off"
        ):
            errors.append(
                ValidationError(
                    field="visualization.debug_level",
