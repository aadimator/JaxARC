...
class AsyncLogger:
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                should_flush = (
                    ...
                    (batch and time.time() - last_flush >= self.config.flush_interval) or
    ...
    def flush(self, timeout: Optional[float] = None) -> bool:
        ...
        while not self.queue.empty():
            if timeout and (time.time() - start_time) > timeout:
                logger.warning("Flush timeout reached, some entries may not be processed")
                return False
            time.sleep(0.1)
    ...
    def shutdown(self, timeout: float = 10.0) -> None:
        ...
        for worker in self.workers:
            worker.join(timeout=timeout/len(self.workers) if self.workers else timeout)
            if worker.is_alive():
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            ...
            all(w.is_alive() for w in self.workers) and
            self.queue.qsize() < self.config.queue_size * 0.9  # Not too full
        )
