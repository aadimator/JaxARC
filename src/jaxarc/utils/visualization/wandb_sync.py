        ...
        for item in self.cache_dir.iterdir():
            if item.is_dir():
                ...
                for subitem in item.iterdir():
                    if subitem.is_dir() and ((subitem / "wandb-metadata.json").exists() or (subitem / "files").exists()):
...
def create_sync_manager(cache_dir: Optional[Path] = None, project_name: str = "jaxarc-experiments") -> WandbSyncManager:
