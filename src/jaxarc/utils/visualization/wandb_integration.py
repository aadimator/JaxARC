...
class WandbConfig:
    ...
    auto_offline_on_error: bool = True  # Automatically switch to offline mode on network errors
    
    # Offline caching settings
    offline_cache_dir: Optional[str] = None  # Directory for offline cache (auto-generated if None)
...
class WandbIntegration:
        ...
        try:
            if isinstance(image, (str, Path)):
                ...
                if len(image.shape) == 2:
                    ...
                    if image.shape[2] == 3:
                        # RGB
                        return PILImage.fromarray((image * 255).astype(np.uint8), mode='RGB')
                    elif image.shape[2] == 4:
                        # RGBA
                        return PILImage.fromarray((image * 255).astype(np.uint8), mode='RGBA')
    ...
    def _optimize_image(self, pil_image: Any, PILImage: Any) -> Any:
        ...
        try:
            ...
            if current_width > max_width or current_height > max_height:
                ...
                logger.debug(f"Resized image from {current_width}x{current_height} to {new_width}x{new_height}")
            
            # Convert to RGB if needed (for PNG format compatibility)
            if self.config.image_format in ("png", "both") and pil_image.mode not in ("RGB", "RGBA", "L"):
    ...
    def _generate_experiment_group(self, experiment_config: Dict[str, Any]) -> Optional[str]:
            ...
            if "debug" in experiment_config:
                ...
                if isinstance(debug_config, dict):
                    ...
                    if debug_level in ("off", "minimal"):
                        ...
                        return "debugging"
                elif isinstance(debug_config, str) and debug_config in ("off", "minimal"):
                    return "training"
                else:
                    return "debugging"
    ...
    def _generate_run_name(self, experiment_config: Dict[str, Any], provided_name: Optional[str]) -> Optional[str]:
    ...
    def _is_network_error(self, exception: Exception) -> bool:
        ...
        network_keywords = [
            ...
            'dns', 'resolve', 'refused', 'reset', 'broken pipe'
        ]
        
        return any(keyword in error_msg for keyword in network_keywords)
    
    def _cache_log_entry(self, data: Dict[str, Any], step: Optional[int] = None) -> None:
    ...
    def _cleanup_cache(self) -> None:
        ...
        try:
            ...
            if total_size_gb > self.config.max_cache_size_gb:
                logger.info(f"Cache size ({total_size_gb:.2f} GB) exceeds limit ({self.config.max_cache_size_gb} GB). Cleaning up...")
                
                # Sort by timestamp (oldest first)
                sorted_entries = sorted(self._cached_entries, key=lambda x: x['timestamp'])
                
                # Remove oldest entries until under limit
                while total_size_gb > self.config.max_cache_size_gb * 0.8 and sorted_entries:  # Clean to 80% of limit
        ...
        for attempt in range(self.config.retry_attempts):
            try:
                ...
                if is_network_error and self.config.auto_offline_on_error:
                    logger.warning(f"Network error detected: {e}. Switching to offline mode.")
                ...
                if attempt < self.config.retry_attempts - 1:
                    ...
                    logger.warning(f"Wandb log attempt {attempt + 1} failed: {e}. Retrying in {actual_delay:.1f}s...")
                    ...
                    if is_network_error:
                        logger.info("Caching entry for offline sync due to network error")
                        self._cache_log_entry(data, step)
                        return True
            ...
            for entry_meta in batch:
                try:
                    ...
                    if not cache_file.exists():
                        logger.warning(f"Cache file not found: {entry_meta['filename']}")
                        failed_count += 1
                        continue
    ...
    def check_network_connectivity(self) -> bool:
        ...
        try:
            ...
            sock = socket.create_connection(("api.wandb.ai", 443), timeout=self.config.network_timeout)
            sock.close()
            return True
    ...
    def get_offline_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'cache_directory': str(self._offline_cache_dir) if self._offline_cache_dir else None,
            'network_connectivity': self.check_network_connectivity() if self._wandb_available else False
        }
    ...
    def clear_offline_cache(self, confirm: bool = False) -> bool:
        ...
        """
        if not confirm:
            logger.warning("Cache clear requested but not confirmed. Set confirm=True to actually clear.")
            return False
        ...
        if self._offline_mode_active:
            ...
            if self.config.auto_sync_on_reconnect and self._cached_entries:
                ...
                if sync_result['success']:
                    logger.info(f"Successfully synced {sync_result['synced_count']} cached entries")
                else:
                    logger.warning(f"Sync partially failed: {sync_result['failed_count']} entries failed")
