...
from ...types import Grid, JaxArcTask
from ..logging.structured_logger import EpisodeLogEntry, LoggingConfig, StepLogEntry, StructuredLogger
from .core import draw_grid_svg, draw_rl_step_svg
from .enhanced_visualizer import EnhancedVisualizer, VisualizationConfig
...
class ReplayConfig:
    ...
    def __post_init__(self):
        if self.comparison_metrics is None:
            object.__setattr__(self, 'comparison_metrics', ['reward', 'similarity', 'steps'])
...
class EpisodeReplaySystem:
    ...
    def __init__(
        ...
        logger.info(f"EpisodeReplaySystem initialized with output dir: {self.output_dir}")
    
    def load_episode(self, episode_num: int, use_cache: bool = True) -> Optional[EpisodeLogEntry]:
    ...
    def validate_episode_integrity(self, episode: EpisodeLogEntry) -> ReplayValidationResult:
        ...
        if episode.total_steps != len(episode.steps):
            errors.append(f"Step count mismatch: expected {episode.total_steps}, got {len(episode.steps)}")
        ...
        if actual_step_nums != expected_step_nums:
            errors.append(f"Step sequence invalid: expected {expected_step_nums}, got {actual_step_nums}")
        
        # Timestamp validation
        prev_timestamp = episode.start_timestamp
        for i, step in enumerate(episode.steps):
            if step.timestamp < prev_timestamp:
                warnings.append(f"Step {i} timestamp {step.timestamp} is before previous {prev_timestamp}")
    ...
    def reconstruct_state_from_log(
        ...
        """
        try:
            state_data = step_log.before_state if use_before_state else step_log.after_state
            ...
            logger.error(f"Failed to reconstruct state for step {step_log.step_num}: {e}")
            return None
    
    def _reconstruct_full_state(self, state_data: Dict[str, Any]) -> Optional[ArcEnvState]:
    ...
    def regenerate_step_visualization(
        ...
        if output_path is None:
            output_path = (
                self.output_dir / f"episode_{episode_num:04d}" / 
        ...
        try:
            ...
            before_state = self.reconstruct_state_from_log(step_log, use_before_state=True)
            after_state = self.reconstruct_state_from_log(step_log, use_before_state=False)
            ...
            logger.debug(f"Regenerated visualization for step {step_log.step_num}: {output_path}")
    ...
    def replay_episode(
        ...
        regenerate_visualizations = (
            regenerate_visualizations if regenerate_visualizations is not None 
        ...
        if validate:
            ...
            if not validation_result.is_valid:
                logger.error(f"Episode {episode_num} failed validation: {validation_result.errors}")
                if not self.config.validate_integrity:  # Continue if validation not required
                    return None
        ...
        summary = {
            ...
            'start_time': time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(episode.start_timestamp)),
            'end_time': time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(episode.end_timestamp)),
        ...
        logger.info(f"Episode {episode_num} replay completed: {len(visualization_paths)} visualizations generated")
        return summary
    ...
    def get_episode_summaries(self, episode_nums: Optional[List[int]] = None) -> List[Dict[str, Any]]:
    ...
    def find_episodes_by_criteria(
        ...
        for summary in summaries:
            ...
            if max_reward is not None and summary['total_reward'] > max_reward:
                continue
            if min_similarity is not None and summary['final_similarity'] < min_similarity:
                continue
            if max_similarity is not None and summary['final_similarity'] > max_similarity:
                continue
