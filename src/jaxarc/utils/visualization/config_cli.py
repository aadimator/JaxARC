...
from .config_composition import create_config_composer, get_config_help, quick_compose
...
def validate(
    config_file: Optional[Path] = typer.Argument(
        None, help="Path to config file to validate"
    ),
    config_name: str = typer.Option(
        "config", help="Config name to compose and validate"
    ),
    overrides: Optional[List[str]] = typer.Option(
        None, "--override", "-o", help="Configuration overrides"
    ),
) -> None:
    ...
    try:
        if config_file:
            ...
            try:
                config = composer.compose_config(
...
def compose(
    ...
    debug_level: str = typer.Option(
        "standard", help="Debug level (off, minimal, standard, verbose, full)"
    ),
    storage_type: str = typer.Option(
        "development", help="Storage type (development, research, production)"
    ),
    logging_type: str = typer.Option(
        "local_only", help="Logging type (local_only, wandb_basic, wandb_full)"
    ),
    overrides: Optional[List[str]] = typer.Option(
        None, "--override", "-o", help="Additional overrides"
    ),
    output: Optional[Path] = typer.Option(
        None, "--output", "-f", help="Save composed config to file"
    ),
    validate_config: bool = typer.Option(
        True, "--validate/--no-validate", help="Validate composed config"
    ),
) -> None:
    ...
    try:
        ...
        if hasattr(config, "visualization"):
            ...
            if hasattr(config.visualization, "output_formats"):
                table.add_row(
                    "Output Formats", ", ".join(config.visualization.output_formats)
                )
...
def create_preset(
    ...
    output_dir: Optional[Path] = typer.Option(
        None, help="Output directory (default: conf/presets)"
    ),
) -> None:
