...
from ..logging.structured_logger import EpisodeLogEntry, StepLogEntry
...
class EpisodeAnalysisTools:
    ...
    def __init__(
        self,
        replay_system: EpisodeReplaySystem,
        ...
        logger.info(f"EpisodeAnalysisTools initialized with output dir: {self.output_dir}")
    
    def analyze_performance_metrics(
        self,
        episode_nums: Optional[List[int]] = None
    ) -> PerformanceMetrics:
        ...
        best_idx = max(range(len(summaries)), key=lambda i: summaries[i]['final_similarity'])
        worst_idx = min(range(len(summaries)), key=lambda i: summaries[i]['final_similarity'])
        ...
        logger.info(f"Performance analysis complete: {metrics.success_rate:.1%} success rate")
        return metrics
    ...
    def analyze_failure_modes(
        ...
        failure_episodes = [
            s['episode_num'] for s in summaries 
        ...
        analysis = FailureModeAnalysis(
            ...
            average_failure_similarity=statistics.mean(failure_similarities) if failure_similarities else 0.0,
        ...
        logger.info(f"Failure mode analysis complete: {len(failure_episodes)} failures analyzed")
        return analysis
    ...
    def compare_episodes(
        ...
        """
        if metrics is None:
            metrics = self.config.comparison_metrics or ['reward', 'similarity', 'steps']
        
        summaries = []
        for episode_num in episode_nums:
            summary = self.replay_system.structured_logger.get_episode_summary(episode_num)
            if summary:
        ...
        comparison = {
            'episodes': [s['episode_num'] for s in summaries],
            'metrics': {}
        }
        ...
        for metric in metrics:
            if metric == 'reward':
                ...
                comparison['metrics']['reward'] = {
                    ...
                    'worst_episode': summaries[values.index(min(values))]['episode_num'],
        ...
        # Save comparison
        comparison_path = self.output_dir / f"episode_comparison_{len(episode_nums)}_episodes.json"
    ...
    def generate_step_by_step_analysis(
        ...
        for i, step in enumerate(episode.steps):
            ...
            if abs(similarity_change) > 0.1:
                analysis['key_moments'].append({
                    'step': step.step_num,
                    'type': 'similarity_jump' if similarity_change > 0 else 'similarity_drop',
        ...
        # Save analysis
        analysis_path = self.output_dir / f"step_analysis_episode_{episode_num:04d}.json"
        with open(analysis_path, 'w', encoding='utf-8') as f:
    ...
    def generate_performance_plots(
        ...
        ax1.plot(episode_nums_sorted, rewards_sorted, 'b-', alpha=0.7, label='Total Reward')
        ax1.set_ylabel('Total Reward')
        ax1.set_title('Performance Over Episodes')
        ax1.grid(True, alpha=0.3)
        ax1.legend()
        
        ax2.plot(episode_nums_sorted, similarities_sorted, 'r-', alpha=0.7, label='Final Similarity')
    ...
    def export_analysis_report(
        ...
        logger.info(f"Generating comprehensive analysis report for {len(episode_nums)} episodes")
        ...
        report = {
            ...
            'episode_range': f"{min(episode_nums)}-{max(episode_nums)}" if episode_nums else "none",
            'performance_metrics': {
            ...
            'failure_analysis': {
                ...
                'failure_rate': len(failure_analysis.failure_episodes) / len(episode_nums) if episode_nums else 0,
