    ...
    def _validate_config(self) -> None:
        ...
        if self.run_name is not None:
            ...
            if any(char in self.run_name for char in invalid_chars):
                raise ValueError(f"run_name contains invalid characters: {invalid_chars}")
...
class EpisodeManager:
    ...
    def start_new_run(self, run_name: str | None = None) -> Path:
        ...
        logger.info(f"Started new run: {self.current_run_name} at {self.current_run_dir}")
    ...
    def start_new_episode(self, episode_num: int) -> Path:
        ...
        if episode_num >= self.config.max_episodes_per_run:
            raise ValueError(f"episode_num {episode_num} exceeds max_episodes_per_run {self.config.max_episodes_per_run}")
        ...
        try:
            ...
            logger.error(f"Failed to create episode directory {self.current_episode_dir}: {e}")
            raise
    ...
    def get_current_run_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "episode_dir": str(self.current_episode_dir) if self.current_episode_dir else None,
        }
    
    def list_episodes_in_run(self, run_dir: Path | None = None) -> list[tuple[int, Path]]:
    ...
    def cleanup_old_data(self) -> None:
        ...
        if current_usage <= self.config.max_storage_gb:
            logger.debug(f"Storage usage {current_usage:.2f}GB is within limit {self.config.max_storage_gb}GB")
            return
        
        logger.info(f"Storage usage {current_usage:.2f}GB exceeds limit {self.config.max_storage_gb}GB - starting cleanup")
