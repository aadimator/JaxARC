...
def plot_mac_evolution():
    ...
    if all(v is not None for v in single_values):
        bars1 = ax1.bar(phases, single_values,
        ...
        for bar, value in zip(bars1, single_values):
            ...
            ax1.text(bar.get_x() + bar.get_width()/2, height + height*0.03,
                    f'{value:,.0f}\nSPS', ha='center', va='bottom',
                    fontweight='bold', fontsize=12, color='#2D3748',
                    bbox=dict(boxstyle='round,pad=0.4', facecolor='white',
                             edgecolor='#CBD5E0', alpha=0.9, linewidth=1))

        ax1.set_title('Single Environment Performance Evolution\nMac M2 Pro Development Progress',
                     fontweight='bold', pad=25, color='#2D3748')
        ...
        ax1.text(0.5, 0.5, 'Single Environment Data\nNot Available',
                transform=ax1.transAxes, ha='center', va='center',
                fontsize=14, fontweight='bold')

    # Plot 2: Batch Scaling Evolution
    if baseline_batches and phase1_batches and phase2_batches:
        ax2.plot(baseline_batches, baseline_sps, 'o-', linewidth=3.5, markersize=9,
                label='Baseline', color=MAC_EVOLUTION_COLORS['Baseline'], alpha=0.9,
                markerfacecolor='white', markeredgewidth=2, markeredgecolor=MAC_EVOLUTION_COLORS['Baseline'])
        ax2.plot(phase1_batches, phase1_sps, 's-', linewidth=3.5, markersize=9,
                label='Phase 1', color=MAC_EVOLUTION_COLORS['Phase 1'], alpha=0.9,
                markerfacecolor='white', markeredgewidth=2, markeredgecolor=MAC_EVOLUTION_COLORS['Phase 1'])
        ax2.plot(phase2_batches, phase2_sps, '^-', linewidth=3.5, markersize=9,
                label='Phase 2', color=MAC_EVOLUTION_COLORS['Phase 2'], alpha=0.9,
                markerfacecolor='white', markeredgewidth=2, markeredgecolor=MAC_EVOLUTION_COLORS['Phase 2'])

        # Add value annotations for higher batch sizes
        annotation_batches = [1000, 5000, 10000]
        for batches, sps_values, color in [(baseline_batches, baseline_sps, MAC_EVOLUTION_COLORS['Baseline']),
            ...
            for i, (batch, sps) in enumerate(zip(batches, sps_values)):
                if batch in annotation_batches:
                    ax2.annotate(f'{sps:,.0f}', (batch, sps),
    ...
    if all(v is not None for v in single_values):
        for phase, value in zip(phases, single_values):
            improvement = ((value / baseline_single - 1) * 100) if phase != 'Baseline' else 0
            print(f"{phase:>10}: {value:>8,.0f} SPS" +
    ...
    if baseline_sps and phase1_sps and phase2_sps:
        ...
        print(f"{'Phase 1':>10}: {phase1_peak:>10,.0f} SPS (+{phase1_batch_improvement:.1f}%)")
        phase2_batch_improvement = (phase2_peak / baseline_peak - 1) * 100
        print(f"{'Phase 2':>10}: {phase2_peak:>10,.0f} SPS (+{phase2_batch_improvement:.1f}%)")
...
def plot_mac_evolution_linear(baseline_data, phase1_data, phase2_data):
    ...
    if baseline_batches and phase1_batches and phase2_batches:
        ...
        for batches, sps_values, phase, color in [
            (baseline_batches, baseline_sps, 'Baseline', MAC_EVOLUTION_COLORS['Baseline']),
            ...
            filtered_data = [(b, s) for b, s in zip(batches, sps_values) if b <= max_batch_for_linear]
            if filtered_data:
                filtered_batches, filtered_sps = zip(*filtered_data)
                marker = 'o' if phase == 'Baseline' else ('s' if phase == 'Phase 1' else '^')
                ax1.plot(filtered_batches, filtered_sps, marker + '-', linewidth=3.5, markersize=9,
                        label=phase, color=color, alpha=0.9,
                ...
                for batch, sps in zip(filtered_batches, filtered_sps):
                    if batch in [1000, 2000] or batch == max(filtered_batches):
                        ax1.annotate(f'{sps:,.0f}', (batch, sps),
                                   xytext=(5, 10), textcoords='offset points',
                                   ha='left', va='bottom', fontsize=9, fontweight='bold',
                                   color=color, alpha=0.8,
                                   bbox=dict(boxstyle='round,pad=0.3', facecolor='white',
        ...
        ax1.set_ylabel('Steps Per Second', fontweight='bold', color='#2D3748')
        ax1.set_title('Mac Evolution (Small Batches)\nLinear Scale View',
                     fontweight='bold', pad=25, color='#2D3748')
        ax1.yaxis.set_major_formatter(FuncFormatter(format_sps))
        ax1.legend(framealpha=0.95, fontsize=12, loc='upper left',
                  fancybox=True, shadow=True, edgecolor='#CBD5E0')
    ...
    if baseline_batches and phase1_batches and phase2_batches:
        for batches, sps_values, phase, color in [
            ...
            marker = 'o' if phase == 'Baseline' else ('s' if phase == 'Phase 1' else '^')
            ax2.plot(batches, sps_values, marker + '-', linewidth=3.5, markersize=7,
                    label=phase, color=color, alpha=0.9,
                    markerfacecolor='white', markeredgewidth=2, markeredgecolor=color)
            ...
            ax2.annotate(f'{peak_sps:,.0f}', (peak_batch, peak_sps),
                       xytext=(5, 10), textcoords='offset points',
        ...
        ax2.set_ylabel('Steps Per Second', fontweight='bold', color='#2D3748')
        ax2.set_title('Mac Evolution (Full Range)\nLinear Scale View',
                     fontweight='bold', pad=25, color='#2D3748')
        ax2.yaxis.set_major_formatter(FuncFormatter(format_sps))
        ax2.xaxis.set_major_formatter(FuncFormatter(format_batch_size))
        ax2.legend(framealpha=0.95, fontsize=12, loc='upper left',
    ...
    plt.suptitle('JaxARC Mac Evolution - Linear Scale Views\nDetailed Development Progress',
                 fontsize=18, fontweight='bold', y=0.96)
    plt.tight_layout()
    plt.show()
...
def plot_hardware_comparison():
    ...
    if any(perf > 0 for perf in single_performance):
        bars1 = ax1.bar(hardware_names, single_performance,
                       color=[HARDWARE_COLORS[name] for name in hardware_names],
                       alpha=0.85, edgecolor='#2D3748', linewidth=1.5, width=0.6)
        ...
        ax1.set_title('Single Environment Performance\nPhase 2 Hardware Comparison',
                     fontweight='bold', pad=25, color='#2D3748')
        ax1.set_ylabel('Steps Per Second', fontweight='bold', color='#2D3748')
    ...
    for name, data in hardware_data.items():
        ...
        if batches and sps_values:
            ax2.plot(batches, sps_values, 'o-', linewidth=3.5, markersize=8,
                    label=name, color=HARDWARE_COLORS[name], alpha=0.9,
            ...
            for batch, sps in zip(batches, sps_values):
                if batch in annotation_batches or batch == max(batches):
                    ax2.annotate(f'{sps:,.0f}', (batch, sps),
                               xytext=(5, 10), textcoords='offset points',
                               ha='left', va='bottom', fontsize=9, fontweight='bold',
                               color=HARDWARE_COLORS[name], alpha=0.8,
                               bbox=dict(boxstyle='round,pad=0.3', facecolor='white',
                                        edgecolor=HARDWARE_COLORS[name], alpha=0.8, linewidth=1))

    ax2.set_xlabel('Batch Size (log scale)', fontweight='bold', color='#2D3748')
    ax2.set_ylabel('Steps Per Second', fontweight='bold', color='#2D3748')
    ax2.set_title('Batch Performance Scaling\nPhase 2 Hardware Comparison',
                 fontweight='bold', pad=25, color='#2D3748')
    ax2.yaxis.set_major_formatter(FuncFormatter(format_sps))
    ax2.xaxis.set_major_formatter(FuncFormatter(format_batch_size))
    ax2.legend(framealpha=0.95, fontsize=12, loc='upper left',
    ...
    plt.suptitle('JaxARC Phase 2 Hardware Comparison\nMac M2 Pro vs RTX 3090 vs H100',
                 fontsize=18, fontweight='bold', y=0.96)
    plt.tight_layout()
    plt.show()
...
def plot_hardware_comparison_linear(hardware_data):
    ...
    for name, data in hardware_data.items():
        ...
        if batches and sps_values:
            ...
            if filtered_data:
                filtered_batches, filtered_sps = zip(*filtered_data)
                ax1.plot(filtered_batches, filtered_sps, 'o-', linewidth=3.5, markersize=8,
                        label=name, color=HARDWARE_COLORS[name], alpha=0.9,
                        markerfacecolor='white', markeredgewidth=2,
                        markeredgecolor=HARDWARE_COLORS[name])
    ...
    ax1.set_xlabel('Batch Size (linear scale)', fontweight='bold', color='#2D3748')
    ax1.set_ylabel('Steps Per Second', fontweight='bold', color='#2D3748')
    ax1.set_title('Batch Performance Scaling (Small Batches)\nLinear Scale View',
                 fontweight='bold', pad=25, color='#2D3748')
    ax1.yaxis.set_major_formatter(FuncFormatter(format_sps))
    ax1.legend(framealpha=0.95, fontsize=12, loc='upper left',
              fancybox=True, shadow=True, edgecolor='#CBD5E0')
    ...
    for name, data in hardware_data.items():
        batches, sps_values = extract_batch_performance(data)
        if batches and sps_values:
            ax2.plot(batches, sps_values, 'o-', linewidth=3.5, markersize=6,
                    label=name, color=HARDWARE_COLORS[name], alpha=0.9,
                    markerfacecolor='white', markeredgewidth=2,
                    markeredgecolor=HARDWARE_COLORS[name])
            ...
            ax2.annotate(f'{peak_sps:,.0f}', (peak_batch, peak_sps),
                       xytext=(5, 10), textcoords='offset points',
                       ha='left', va='bottom', fontsize=10, fontweight='bold',
                       color=HARDWARE_COLORS[name], alpha=0.9,
                       bbox=dict(boxstyle='round,pad=0.3', facecolor='white',
                                edgecolor=HARDWARE_COLORS[name], alpha=0.9, linewidth=1.5))

    ax2.set_xlabel('Batch Size (linear scale)', fontweight='bold', color='#2D3748')
    ax2.set_ylabel('Steps Per Second', fontweight='bold', color='#2D3748')
    ax2.set_title('Batch Performance Scaling (Full Range)\nLinear Scale View',
                 fontweight='bold', pad=25, color='#2D3748')
    ax2.yaxis.set_major_formatter(FuncFormatter(format_sps))
    ax2.xaxis.set_major_formatter(FuncFormatter(format_batch_size))
    ax2.legend(framealpha=0.95, fontsize=12, loc='upper left',
    ...
    plt.suptitle('JaxARC Hardware Comparison - Linear Scale Views\nDetailed Performance Analysis',
                 fontsize=18, fontweight='bold', y=0.96)
    plt.tight_layout()
    plt.show()
...
def plot_acceleration_factors():
    ...
    if rtx_speedups:
        bars1 = ax.bar(x - width/2, rtx_speedups, width,
                      label='RTX 3090', color=HARDWARE_COLORS['RTX 3090'],
                      alpha=0.85, edgecolor='#2D3748', linewidth=1.5)
        ...
        for bar, speedup in zip(bars1, rtx_speedups):
            ...
            ax.text(bar.get_x() + bar.get_width()/2, height + 0.05,
                    ...
                    fontweight='bold', fontsize=10, color='#2D3748',
                    bbox=dict(boxstyle='round,pad=0.2', facecolor='white',
                             edgecolor='#CBD5E0', alpha=0.9))

    if h100_speedups:
        bars2 = ax.bar(x + width/2, h100_speedups, width,
                      label='H100', color=HARDWARE_COLORS['H100'],
                      alpha=0.85, edgecolor='#2D3748', linewidth=1.5)
        ...
        for bar, speedup in zip(bars2, h100_speedups):
            ...
            ax.text(bar.get_x() + bar.get_width()/2, height + 0.05,
                    f'{speedup:.1f}×', ha='center', va='bottom',
                    fontweight='bold', fontsize=11, color='#2D3748',
                    bbox=dict(boxstyle='round,pad=0.3', facecolor='white',
                             edgecolor='#CBD5E0', alpha=0.9))
    ...
    ax.set_title('GPU Acceleration Factors\nPerformance Multiplier vs Mac M2 Pro Baseline',
                 fontweight='bold', pad=25, color='#2D3748')
    ax.legend(framealpha=0.95, fontsize=12, loc='upper left',
              fancybox=True, shadow=True, edgecolor='#CBD5E0')
...
def plot_h100_comparison():
    ...
    if h100_batches and h100_scan_batches:
        ax1.plot(h100_batches, h100_sps, 'o-', linewidth=3.5, markersize=8,
                label='H100 Regular', color=HARDWARE_COLORS['H100'], alpha=0.9,
                markerfacecolor='white', markeredgewidth=2, markeredgecolor=HARDWARE_COLORS['H100'])
        ax1.plot(h100_scan_batches, h100_scan_sps, 's-', linewidth=3.5, markersize=8,
                label='H100 Scan', color=HARDWARE_COLORS['H100 Scan'], alpha=0.9,
                markerfacecolor='white', markeredgewidth=2, markeredgecolor=HARDWARE_COLORS['H100 Scan'])

        # Add value annotations for higher batch sizes
        annotation_batches = [10000, 50000, 100000]
        for batches, sps_values, color in [(h100_batches, h100_sps, HARDWARE_COLORS['H100']),
        ...
        ax1.set_ylabel('Steps Per Second', fontweight='bold', color='#2D3748')
        ax1.set_title('H100 Regular vs Scan\nLog Scale Comparison', fontweight='bold', pad=20, color='#2D3748')
        ax1.yaxis.set_major_formatter(FuncFormatter(format_sps))
    ...
    if h100_batches and h100_scan_batches:
        h100_filtered = [(b, s) for b, s in zip(h100_batches, h100_sps) if b <= max_batch_linear]
        scan_filtered = [(b, s) for b, s in zip(h100_scan_batches, h100_scan_sps) if b <= max_batch_linear]

        if h100_filtered:
            h100_filt_batches, h100_filt_sps = zip(*h100_filtered)
            ax2.plot(h100_filt_batches, h100_filt_sps, 'o-', linewidth=3.5, markersize=8,
                    label='H100 Regular', color=HARDWARE_COLORS['H100'], alpha=0.9,
                    markerfacecolor='white', markeredgewidth=2, markeredgecolor=HARDWARE_COLORS['H100'])

        if scan_filtered:
            scan_filt_batches, scan_filt_sps = zip(*scan_filtered)
            ax2.plot(scan_filt_batches, scan_filt_sps, 's-', linewidth=3.5, markersize=8,
                    label='H100 Scan', color=HARDWARE_COLORS['H100 Scan'], alpha=0.9,
                    markerfacecolor='white', markeredgewidth=2, markeredgecolor=HARDWARE_COLORS['H100 Scan'])
            ...
            for batch, sps in zip(scan_filt_batches, scan_filt_sps):
                if batch in [1000, 5000, 10000]:
                    ax2.annotate(f'{sps:,.0f}', (batch, sps),
                               xytext=(5, 10), textcoords='offset points',
                               ha='left', va='bottom', fontsize=9, fontweight='bold',
                               color=HARDWARE_COLORS['H100 Scan'], alpha=0.8,
                               bbox=dict(boxstyle='round,pad=0.3', facecolor='white',
                                        edgecolor=HARDWARE_COLORS['H100 Scan'], alpha=0.8, linewidth=1))
            ...
            for batch, sps in zip(h100_filt_batches, h100_filt_sps):
                if batch in [1000, 5000, 10000]:
                    ax2.annotate(f'{sps:,.0f}', (batch, sps),
                               ...
                               color=HARDWARE_COLORS['H100'], alpha=0.8,
                               bbox=dict(boxstyle='round,pad=0.3', facecolor='white',
                                        edgecolor=HARDWARE_COLORS['H100'], alpha=0.8, linewidth=1))

        ax2.set_xlabel('Batch Size (linear scale)', fontweight='bold', color='#2D3748')
        ax2.set_ylabel('Steps Per Second', fontweight='bold', color='#2D3748')
        ax2.set_title('H100 Regular vs Scan (≤10K batches)\nLinear Scale Comparison', fontweight='bold', pad=20, color='#2D3748')
        ax2.yaxis.set_major_formatter(FuncFormatter(format_sps))
        ax2.xaxis.set_major_formatter(FuncFormatter(format_batch_size))
    ...
    if h100_batches and h100_scan_batches:
        ...
        if speedups:
            bars = ax3.bar(range(len(speedups)), speedups,
                          color=HARDWARE_COLORS['H100 Scan'], alpha=0.85,
                          edgecolor='#2D3748', linewidth=1.5)
            ...
            ax3.set_ylabel('Speedup Factor (Scan vs Regular)', fontweight='bold', color='#2D3748')
            ax3.set_title('H100 Scan Speedup vs Regular\nPerformance Multiplier', fontweight='bold', pad=20, color='#2D3748')
    ...
    if h100_sps and h100_scan_sps:
        ...
        bars1 = ax4.bar(x - width/2, h100_vals, width, label='H100 Regular',
                       color=HARDWARE_COLORS['H100'], alpha=0.85, edgecolor='#2D3748')
        bars2 = ax4.bar(x + width/2, scan_vals, width, label='H100 Scan',
                       color=HARDWARE_COLORS['H100 Scan'], alpha=0.85, edgecolor='#2D3748')
        ...
        ax4.text(0.5, 0.95, f'Scan is {improvement:.1f}× faster\n({scan_peak:,.0f} vs {h100_peak:,.0f} SPS)',
        ...
        ax4.set_title('Peak Performance Summary\nH100 Regular vs Scan', fontweight='bold', pad=20, color='#2D3748')
        ax4.yaxis.set_major_formatter(FuncFormatter(format_sps))
        ax4.legend(framealpha=0.95, fontsize=12)
        ax4.grid(True, alpha=0.2, axis='y')

    plt.suptitle('JaxARC H100 Performance Comparison\nRegular Implementation vs Optimized Scan',
