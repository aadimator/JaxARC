...
def create_sample_episodes(logger: StructuredLogger) -> None:
    ...
    for step in range(3):
        logger.log_step(
            step_num=step,
            before_state={"type": "ArcEnvState", "step_count": step},
            action={"operation": 10, "selection": [[0, 1], [1, 0]]},  # Repetitive action
            after_state={"type": "ArcEnvState", "step_count": step + 1},
...
def demonstrate_replay_functionality(replay_system: EpisodeReplaySystem) -> None:
    ...
    for summary in summaries:
        print(f"  Episode {summary['episode_num']}: "
...
def demonstrate_analysis_functionality(analysis_tools: EpisodeAnalysisTools) -> None:
    ...
    print(f"Average similarity: {metrics.average_similarity:.3f} ± {metrics.similarity_std:.3f}")
    ...
    print(f"Average failure similarity: {failure_analysis.average_failure_similarity:.3f}")
        ...
        if step_analysis:
            ...
            if step_analysis['key_moments']:
                ...
                for moment in step_analysis['key_moments']:
                    print(f"    Step {moment['step']}: {moment['type']} ({moment['change']:+.3f})")
