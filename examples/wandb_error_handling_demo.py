...
def demo_offline_caching():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch('jaxarc.utils.visualization.wandb_integration.WandbIntegration._initialize_wandb'):
            ...
            print(f"Sync result: {sync_result['synced_count']} synced, {sync_result['failed_count']} failed")
...
def demo_sync_manager():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        run_dir.mkdir()
        (run_dir / "wandb-metadata.json").write_text('{"run_id": "demo-run", "project": "demo-project"}')
        (run_dir / "files").mkdir()
