...
def demonstrate_parser_basics():
    ...
    console.print(
        Panel.fit(
            "[bold blue]🔍 ConceptARC Parser Basics[/bold blue]", border_style="blue"
        )
    )

    # Create ConceptARC configuration
    config = create_config_util(
        max_episode_steps=100, task_split="corpus", success_bonus=15.0
    )
    ...
    try:
        ...
        console.print(
            "[dim]This is expected if you haven't downloaded the ConceptARC dataset yet.[/dim]"
        )
        console.print(
            "[dim]You can download it using: python scripts/download_dataset.py conceptarc[/dim]"
        )
    ...
    console.print(
        f"📊 Found [bold cyan]{len(concept_groups)}[/bold cyan] concept groups:"
    )
    ...
    console.print(
        f"  • Total concept groups: [bold]{stats['total_concept_groups']}[/bold]"
    )
...
def demonstrate_concept_exploration(parser: ConceptArcParser, concept: str):
    ...
    try:
        ...
        console.print(
            f"📋 [bold]{concept}[/bold] concept contains [cyan]{len(concept_tasks)}[/cyan] tasks:"
        )
...
def demonstrate_batch_processing(parser: ConceptArcParser):
    ...
    console.print(
        f"🔄 Processing tasks from [cyan]{len(concept_groups)}[/cyan] concepts:"
    )
    ...
    for key, concept in zip(keys, concept_groups):
        ...
        console.print(
            f"  • [yellow]{concept}[/yellow]: {result['train_pairs']} train, {result['test_pairs']} test"
        )
...
def main(
    ...
    interactive: bool = typer.Option(
        False, "--interactive", "-i", help="Run interactive exploration mode"
    ),
    visualize: bool = typer.Option(
        False, "--visualize", "-v", help="Save task visualizations to SVG files"
    ),
    run_episode: bool = typer.Option(
        False, "--run-episode", "-e", help="Run a complete environment episode"
    ),
    batch_demo: bool = typer.Option(
        False, "--batch", "-b", help="Demonstrate batch processing"
    ),
