...
def demo_basic_factory_usage():
    ...
    print(
        f"Development config: {dev_config.environment.max_episode_steps} steps, "
    ...
    print(
        f"Research config: {research_config.environment.max_episode_steps} steps, "
    ...
    print(
        f"Production config: {prod_config.environment.max_episode_steps} steps, "
...
def demo_hydra_conversion():
    ...
    hydra_dict = {
        "environment": {"max_episode_steps": 120, "debug_level": "standard"},
        "dataset": {"dataset_name": "concept-arc", "task_split": "corpus"},
        "visualization": {"enabled": True, "level": "full"},
        "wandb": {"enabled": True, "project_name": "hydra-experiment"},
    }
...
def demo_preset_with_overrides():
    ...
    print("Custom mini_arc preset:")
    print(
        f"  - Episode steps: {custom_mini.environment.max_episode_steps} (overridden)"
    )
    print(f"  - Dataset: {custom_mini.dataset.dataset_name}")
    print(
        f"  - Grid size: {custom_mini.dataset.max_grid_height}x{custom_mini.dataset.max_grid_width}"
    )
...
def demo_config_validation():
    ...
    config = ConfigFactory.create_research_config(
        max_episode_steps=200, wandb_enabled=True
    )
...
def demo_yaml_serialization():
    ...
    config = ConfigFactory.create_development_config(
        max_episode_steps=50, dataset_name="mini-arc", visualization_level="standard"
    )
