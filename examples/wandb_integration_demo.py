    ...
    if integration.is_available:
        ...
        if result:
            ...
            for step in range(10):
                metrics = {
                    "reward": float(jnp.sin(step * 0.5) + jax.random.normal(jax.random.PRNGKey(step))),
                    "loss": float(1.0 / (step + 1) + 0.1 * jax.random.normal(jax.random.PRNGKey(step + 100))),
...
def demo_error_handling() -> None:
    ...
    print(f"Initialize run (no wandb): {integration.initialize_run({'test': 'config'})}")
    print(f"Log step (no wandb): {integration.log_step(1, {'reward': 0.5})}")
    print(f"Log episode (no wandb): {integration.log_episode_summary(1, {'reward': 10})}")
    print(f"Log config update (no wandb): {integration.log_config_update({'lr': 0.01})}")
