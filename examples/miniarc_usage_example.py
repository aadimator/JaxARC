...
def demonstrate_parser_basics():
    ...
    console.print(Panel.fit(
        "[bold blue]🔍 MiniARC Parser Basics[/bold blue]",
        border_style="blue"
    ))
    ...
    try:
        ...
        console.print("[dim]This is expected if you haven't downloaded the MiniARC dataset yet.[/dim]")
        console.print("[dim]You can download it using: python scripts/download_kaggle_dataset.py download-miniarc[/dim]")
    ...
    console.print(f"  • Max dimensions: [bold]{stats['max_configured_dimensions']}[/bold]")
    console.print(f"  • 5x5 optimized: [bold green]{stats['is_5x5_optimized']}[/bold green]")
...
def demonstrate_5x5_optimization_benefits():
    ...
    console.print(f"  • Memory ratio: [bold]{standard_grid.size // miniarc_grid.size}:1[/bold]")
    ...
    console.print(f"  • Throughput improvement: [bold]{miniarc_batch_size // standard_batch_size}x[/bold]")
...
def demonstrate_rapid_prototyping_workflow(parser):
    ...
    for iteration in range(3):
        ...
        for step in range(5):
            action = {
                "selection": jnp.array([step % 5, step % 5], dtype=jnp.int32),  # Point action
    ...
    console.print("[dim]Compare this to standard ARC which might take minutes or hours![/dim]")
...
def demonstrate_performance_comparison():
    ...
    console.print(f"  • MiniARC grid size: [green]{miniarc_config.grid.max_grid_height}×{miniarc_config.grid.max_grid_width}[/green]")
    console.print(f"  • Standard ARC grid size: [red]{standard_config.grid.max_grid_height}×{standard_config.grid.max_grid_width}[/red]")
    ...
    speedup = standard_creation_time / miniarc_creation_time if miniarc_creation_time > 0 else 1
    ...
    console.print(f"  ✅ MiniARC episode (10 steps): [green]{miniarc_episode_time:.4f}s[/green]")
    console.print(f"  ✅ Standard episode (10 steps): [red]{standard_episode_time:.4f}s[/red]")
    episode_speedup = standard_episode_time / miniarc_episode_time if miniarc_episode_time > 0 else 1
    ...
    console.print(f"  💾 Memory efficiency: [bold]{memory_ratio:.1f}x less memory[/bold]")
...
def demonstrate_batch_processing(parser):
    ...
    for i, key in enumerate(keys):
        ...
        console.print(f"  Task {i+1}: [cyan]{grid_stats['grid_shape']}[/cyan] grid, "
    ...
    console.print(f"📊 Average time per task: [yellow]{batch_time/batch_size:.4f}s[/yellow]")
...
def main(
    ...
    try:
        ...
        if performance_comparison:
            ...
            console.print(f"  • Environment creation: [green]{perf_results['creation_speedup']:.2f}x faster[/green]")
            console.print(f"  • Episode execution: [green]{perf_results['episode_speedup']:.2f}x faster[/green]")
        ...
        if not any([rapid_prototyping, performance_comparison, batch_processing]):
            ...
            console.print("[dim]  • --performance-comparison: Compare with standard ARC[/dim]")
            console.print("[dim]  • --rapid-prototyping: Show development workflow[/dim]")
            console.print("[dim]  • --batch-processing: Demonstrate batch efficiency[/dim]")
