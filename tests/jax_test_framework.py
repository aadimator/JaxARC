...
class JaxTransformationTester:
    ...
    def __init__(
        self, func: Callable, test_inputs: tuple, test_kwargs: dict | None = None
    ):
        """
    ...
    def test_grad_compatibility(self) -> None:
        ...
        try:
            ...
            if all(
                isinstance(arg, jnp.ndarray) and jnp.issubdtype(arg.dtype, jnp.floating)
                for arg in self.test_inputs
            ):
                baseline = self.get_baseline_result()
                if isinstance(baseline, jnp.ndarray) and baseline.shape == ():
                    ...
                    if len(self.test_inputs) == 1:
                        chex.assert_trees_all_equal_shapes(
                            self.test_inputs[0], grad_result
                        )
                    else:
                        chex.assert_trees_all_equal_shapes(
                            self.test_inputs, grad_result
                        )
...
class JaxPropertyTester:
    ...
    def test_dtype_preservation(
        ...
        if isinstance(result, jnp.ndarray) or hasattr(result, "dtype"):
            assert result.dtype == input_array.dtype

    @staticmethod
    def test_commutativity(
        func: Callable, arg1: Any, arg2: Any, *args, **kwargs
    ) -> None:
    ...
    def test_associativity(
        func: Callable, arg1: Any, arg2: Any, arg3: Any, *args, **kwargs
    ) -> None:
