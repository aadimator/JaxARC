...
def arc_grids(draw) -> Dict[str, jnp.ndarray]:
    ...
    elements = st.integers(min_value=0, max_value=9)
    data = draw(hnp.arrays(dtype=np.int32, shape=shape, elements=elements)).astype(jnp.int32)
    
    # Generate mask
    mask = draw(hnp.arrays(dtype=np.bool_, shape=shape, elements=st.booleans())).astype(jnp.bool_)
...
def arc_task_pairs(draw, max_pairs: int = 5) -> Dict[str, Any]:
    ...
    input_grids_examples = draw(jax_arrays(
        shape=(num_train_pairs, max_height, max_width),
        dtype=jnp.int32,
        min_value=0,
        max_value=9
    ))
    input_masks_examples = draw(jax_arrays(
        shape=(num_train_pairs, max_height, max_width),
        dtype=jnp.bool_
    ))
    output_grids_examples = draw(jax_arrays(
        shape=(num_train_pairs, max_height, max_width),
        dtype=jnp.int32,
        min_value=0,
        max_value=9
    ))
    output_masks_examples = draw(jax_arrays(
        shape=(num_train_pairs, max_height, max_width),
        dtype=jnp.bool_
    ))
    
    # Generate test data
    test_input_grids = draw(jax_arrays(
        shape=(num_test_pairs, max_height, max_width),
        dtype=jnp.int32,
        min_value=0,
        max_value=9
    ))
    test_input_masks = draw(jax_arrays(
        shape=(num_test_pairs, max_height, max_width),
        dtype=jnp.bool_
    ))
    true_test_output_grids = draw(jax_arrays(
        shape=(num_test_pairs, max_height, max_width),
        dtype=jnp.int32,
        min_value=0,
        max_value=9
    ))
    true_test_output_masks = draw(jax_arrays(
        shape=(num_test_pairs, max_height, max_width),
        dtype=jnp.bool_
    ))
...
class JaxPropertyTester:
    ...
    def assert_jit_compatible(func: Callable, *args, **kwargs) -> None:
        ...
        if isinstance(baseline, jnp.ndarray):
            assert jnp.allclose(baseline, jitted_result), "JIT compilation changed function behavior"
        else:
            assert baseline == jitted_result, "JIT compilation changed function behavior"
