...
from jaxarc.envs.grid_operations import fill_color, copy_to_clipboard, paste_from_clipboard
...
class TestJAXTransformationCompatibility:
    ...
    def test_grid_operations_jit_compatibility(self, demo_state):
        ...
        assert jnp.array_equal(new_state.working_grid[0:2, 0:2], expected_grid[0:2, 0:2])
...
class TestPerformanceBenchmarks:
    ...
    def test_state_update_performance_comparison(self, demo_state):
        ...
        print(f"State update performance: eqx.tree_at={equinox_time:.4f}s, replace={replace_time:.4f}s")
    ...
    def test_grid_operations_performance(self, demo_state):
        ...
        print(f"Grid operations performance: {grid_ops_time:.4f}s for {iterations} operation sequences")
    ...
    def test_memory_usage_comparison(self, demo_state):
        ...
        print(f"Memory usage: Equinox={equinox_increase:.1f}MB, Replace={replace_increase:.1f}MB")
        
        # Both should use reasonable memory (less than 100MB for 100 states)
        assert equinox_increase < 100, f"Equinox memory usage too high: {equinox_increase}MB"
        assert replace_increase < 100, f"Replace memory usage too high: {replace_increase}MB"
        ...
        if equinox_increase > 0 and replace_increase > 0:
            ratio = max(equinox_increase, replace_increase) / min(equinox_increase, replace_increase)
    ...
    def test_compilation_time_benchmark(self, demo_state):
        ...
        def complex_equinox_function(state):
            ...
            state = eqx.tree_at(lambda s: s.similarity_score, state, state.similarity_score * 2.0)
...
class TestBackwardCompatibility:
    ...
    def test_field_access_compatibility(self, demo_state):
        ...
        assert hasattr(demo_state.step_count, 'shape') or isinstance(demo_state.step_count, (int, float))
        assert hasattr(demo_state.episode_done, 'shape') or isinstance(demo_state.episode_done, bool)
        assert hasattr(demo_state.similarity_score, 'shape') or isinstance(demo_state.similarity_score, float)
    ...
    def test_functional_api_compatibility(self, config, key):
        ...
        action = {
            'selection': jnp.zeros_like(state.working_grid, dtype=bool).at[0:2, 0:2].set(True),
            'operation': 1
        }
...
class TestEquinoxUtilities:
    ...
    def test_tree_map_with_path_utility(self, demo_state):
        ...
        expected_field_names = [
            'working_grid', 'working_grid_mask', 'target_grid',
            'selected', 'clipboard'  # These are definitely arrays
        ]
