...
class TestEquinoxPerformance:
    ...
    def test_jit_compilation_performance(self, demo_state):
        ...
        print(f"JIT Performance: eqx.tree_at={equinox_time:.4f}s, replace={replace_time:.4f}s")
    ...
    def test_grid_operations_performance(self, demo_state):
        ...
        print(f"Grid operations performance: {grid_ops_time:.4f}s for {iterations} operations")
    ...
    def test_functional_api_performance(self, config, key):
        ...
        for _ in range(5):
            state, _ = arc_reset(key, config)
            action = {
                'selection': jnp.zeros_like(state.working_grid, dtype=jnp.bool_).at[0:2, 0:2].set(True),
                'operation': 1
            }
        ...
        print(f"Functional API performance: {api_time:.4f}s for {iterations} reset-step cycles")
    ...
    def test_batch_operations_performance(self, demo_state):
        ...
        print(f"Batch operations performance: {batch_time:.4f}s for {iterations} batch operations")
    ...
    def test_complex_state_updates_performance(self, demo_state):
        ...
        print(f"Complex updates: eqx.tree_at={equinox_time:.4f}s, replace={replace_time:.4f}s")
