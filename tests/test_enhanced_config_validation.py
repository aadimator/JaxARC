...
class TestValidationUtilities:
    ...
    def test_validate_path_string(self):
        ...
        for char in invalid_chars:
            with pytest.raises(
                ConfigValidationError, match="contains invalid path characters"
            ):
    ...
    def test_validate_operation_list(self):
        ...
        with pytest.raises(
            ConfigValidationError, match="contains duplicate operations"
        ):
    ...
    def test_validate_dataset_name(self):
        ...
        with pytest.raises(
            ConfigValidationError, match="must contain only alphanumeric"
        ):
...
class TestDebugConfigValidation:
    ...
    def test_invalid_debug_config_types(self):
        ...
        with pytest.raises(
            ConfigValidationError, match="log_rl_steps must be a boolean"
        ):
            DebugConfig(log_rl_steps="not_bool")

        # Invalid clear_output_dir type
        with pytest.raises(
            ConfigValidationError, match="clear_output_dir must be a boolean"
        ):
...
class TestRewardConfigValidation:
    ...
    def test_invalid_reward_config_types(self):
        ...
        with pytest.raises(
            ConfigValidationError, match="reward_on_submit_only must be a boolean"
        ):
            RewardConfig(reward_on_submit_only="not_bool")

        # Invalid numeric types
        with pytest.raises(
            ConfigValidationError, match="step_penalty must be a number"
        ):
            RewardConfig(step_penalty="not_number")

        with pytest.raises(
            ConfigValidationError, match="success_bonus must be a number"
        ):
    ...
    def test_reward_config_range_validation(self):
        ...
        with pytest.raises(
            ConfigValidationError, match="step_penalty must be in range"
        ):
            RewardConfig(step_penalty=-15.0)  # Too negative

        with pytest.raises(
            ConfigValidationError, match="success_bonus must be in range"
        ):
            RewardConfig(success_bonus=-200.0)  # Too negative

        with pytest.raises(
            ConfigValidationError, match="similarity_weight must be in range"
        ):
...
class TestDatasetConfigValidation:
    ...
    def test_grid_constraint_validation(self):
        ...
        with pytest.raises(
            ConfigValidationError,
            match="dataset_min_grid_height.*dataset_max_grid_height",
        ):
            DatasetConfig(
...
class TestGridConfigValidation:
    ...
    def test_invalid_grid_dimensions(self):
        ...
        # max < min
        with pytest.raises(
            ConfigValidationError, match="max_grid_height.*min_grid_height"
        ):
            GridConfig(max_grid_height=2, min_grid_height=3)

        with pytest.raises(
            ConfigValidationError, match="max_grid_width.*min_grid_width"
        ):
    ...
    def test_invalid_color_constraints(self):
        ...
        with pytest.raises(
            ConfigValidationError, match="max_colors must be at least 2"
        ):
            GridConfig(max_colors=1)

        # background_color >= max_colors
        with pytest.raises(
            ConfigValidationError, match="background_color.*must be.*max_colors"
        ):
    ...
    def test_grid_dimension_types(self):
        ...
        with pytest.raises(
            ConfigValidationError, match="max_grid_height must be an integer"
        ):
            GridConfig(max_grid_height="not_int")

        # Negative dimensions
        with pytest.raises(
            ConfigValidationError, match="min_grid_height must be positive"
        ):
...
class TestActionConfigValidation:
    ...
    def test_invalid_selection_format(self):
        ...
        with pytest.raises(
            ConfigValidationError, match="selection_format must be one of"
        ):
    ...
    def test_invalid_selection_threshold(self):
        ...
        with pytest.raises(
            ConfigValidationError, match="selection_threshold must be in range"
        ):
    ...
    def test_invalid_num_operations(self):
        ...
        with pytest.raises(
            ConfigValidationError, match="num_operations must be positive"
        ):
    ...
    def test_invalid_allowed_operations(self):
        ...
        with pytest.raises(
            ConfigValidationError, match="allowed_operations.*must be in range"
        ):
    ...
    def test_boolean_field_validation(self):
        ...
        with pytest.raises(
            ConfigValidationError, match="allow_partial_selection must be a boolean"
        ):
            ActionConfig(allow_partial_selection="not_bool")

        with pytest.raises(
            ConfigValidationError, match="validate_actions must be a boolean"
        ):
...
class TestArcEnvConfigValidation:
    ...
    def test_invalid_episode_steps(self):
        ...
        with pytest.raises(
            ConfigValidationError, match="max_episode_steps must be positive"
        ):
    ...
    def test_boolean_field_validation(self):
        ...
        with pytest.raises(
            ConfigValidationError, match="strict_validation must be a boolean"
        ):
...
class TestConfigValidationFunction:
    ...
    def test_invalid_config_validation(self):
        ...
        with pytest.raises(
            ...
            GridConfig(
                max_colors=5, background_color=5
            )  # Invalid: background >= max_colors
