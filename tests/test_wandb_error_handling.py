...
from src.jaxarc.utils.visualization.wandb_integration import WandbConfig, WandbIntegration
from src.jaxarc.utils.visualization.wandb_sync import WandbSyncManager, create_sync_manager
...
class TestWandbErrorHandling:
    ...
    def test_network_error_detection(self):
        ...
        with patch('src.jaxarc.utils.visualization.wandb_integration.WandbIntegration._initialize_wandb'):
            ...
            for error in network_errors:
                assert integration._is_network_error(error), f"Should detect {type(error).__name__} as network error"
            ...
            for error in non_network_errors:
                assert not integration._is_network_error(error), f"Should not detect {type(error).__name__} as network error"
    ...
    def test_offline_cache_management(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('src.jaxarc.utils.visualization.wandb_integration.WandbIntegration._initialize_wandb'):
                ...
                for i in range(10):
                    integration._cache_log_entry({"step": i, "data": f"test_data_{i}"}, step=i)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('src.jaxarc.utils.visualization.wandb_integration.WandbIntegration._initialize_wandb'):
                ...
                for i in range(initial_count):
                    large_data = {"step": i, "large_data": "x" * 1000}  # Large data to trigger cleanup
