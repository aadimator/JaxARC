...
class TestConceptArcConfiguration:
    ...
    def test_conceptarc_missing_required_fields(self):
        ...
        with pytest.raises(
            Exception
        ):  # OmegaConf will raise when accessing missing key
...
class TestFactoryFunctions:
    ...
    def test_create_conceptarc_config_kwargs_override(self):
        ...
        config = create_conceptarc_config(
            max_episode_steps=100, log_operations=True, strict_validation=False
        )
    ...
    def test_create_miniarc_config_kwargs_override(self):
        ...
        config = create_miniarc_config(
            max_episode_steps=60, log_grid_changes=True, allow_invalid_actions=True
        )
...
class TestConfigurationErrorHandling:
    ...
    def test_invalid_reward_configuration(self):
        ...
        config = create_conceptarc_config(
            step_penalty=0.1
        )  # Positive penalty gives warning
...
class TestConfigurationIntegration:
    ...
    def test_conceptarc_config_with_hydra(self):
        ...
        hydra_override = OmegaConf.create(
            {"max_episode_steps": 200, "reward": {"success_bonus": 20.0}}
        )
    ...
    def test_config_validation_comprehensive(self):
        ...
        assert (
            conceptarc_config.grid.background_color < conceptarc_config.grid.max_colors
        )
        assert miniarc_config.grid.background_color < miniarc_config.grid.max_colors

        assert (
            conceptarc_config.grid.min_grid_height
            <= conceptarc_config.grid.max_grid_height
        )
        assert (
            miniarc_config.grid.min_grid_height <= miniarc_config.grid.max_grid_height
        )
