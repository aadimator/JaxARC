...
class TestConfigurationIntegration:
    ...
    def test_debug_verbose_integration(self):
        ...
        config = quick_compose(
            debug_level="verbose", storage_type="research", validate=True
        )
    ...
    def test_storage_compatibility(self):
        ...
        for debug_level in ["off", "minimal", "standard"]:
            config = quick_compose(
                debug_level=debug_level, storage_type="development", validate=True
            )
        ...
        for debug_level in ["verbose", "full"]:
            config = quick_compose(
                debug_level=debug_level, storage_type="research", validate=True
            )
            assert config.storage.max_storage_gb >= 10.0
    ...
    def test_wandb_integration_compatibility(self):
        ...
        config = quick_compose(
            debug_level="standard", logging_type="wandb_basic", validate=True
        )
        ...
        config = quick_compose(
            debug_level="verbose", logging_type="wandb_full", validate=True
        )
    ...
    def test_cross_validation_warnings(self):
        ...
        config = OmegaConf.create(
            {
                "visualization": {"debug_level": "full", "enabled": True},
                "storage": {
    ...
    def test_backward_compatibility(self):
        ...
        minimal_config = OmegaConf.create({"dataset": "arc_agi_1", "seed": 42})
...
class TestConfigMigrator:
    ...
    def test_detect_legacy_config(self):
        ...
        legacy_config = OmegaConf.create(
            {"log_rl_steps": True, "rl_steps_output_dir": "outputs/debug"}
        )
