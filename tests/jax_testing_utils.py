    ...
    def check_shape_consistency(
        ...
        for name, array in arrays.items():
            if name in expected_shapes:
                ...
                assert array.shape == expected, f"Shape mismatch for {name}: expected {expected}, got {array.shape}"
...
class JaxTransformationValidator:
    ...
    def validate_vmap(
        ...
        **kwargs
        ...
        """
        # Get single example
        single_args = tree_map(lambda x: x[0] if isinstance(x, jnp.ndarray) else x, batch_args)
        ...
        first_batch_result = tree_map(
            lambda x: x[0] if isinstance(x, jnp.ndarray) else x, 
            batch_result
        )
    ...
    def validate_pmap(
        ...
        **kwargs
        ...
        device_args = tree_map(
            lambda x: jnp.stack([x] * available_devices) if isinstance(x, jnp.ndarray) else x,
            args
        )
        ...
        **kwargs
        ...
        try:
            ...
            for i in ([argnums] if isinstance(argnums, int) else argnums):
                if i >= len(args) or not isinstance(args[i], jnp.ndarray) or not jnp.issubdtype(args[i].dtype, jnp.floating):
...
class JaxEquinoxTester:
    ...
    def test_module_jax_compatibility(
        module: eqx.Module,
        transform_types: List[str] = ["jit", "vmap"]
    ) -> None:
        """
...
def assert_shapes_match(arrays: Dict[str, jnp.ndarray], expected_shapes: Dict[str, Tuple[int, ...]]) -> None:
