...
class TestLargeEpisodeHandling:
    ...
    def test_large_episode_1000_steps(
        self, performance_visualizer: EnhancedVisualizer
    ) -> None:
        ...
        assert profiler.duration < 30.0, (
            f"1000 steps took too long: {profiler.duration:.2f}s"
        )
        assert profiler.peak_memory_delta < 500.0, (
            f"Memory usage too high: {profiler.peak_memory_delta:.2f}MB"
        )
    ...
    def test_multiple_large_episodes(
        self, performance_visualizer: EnhancedVisualizer
    ) -> None:
        ...
        with (
            ...
            for episode_num in range(1, 6):
                performance_visualizer.start_episode(
                    episode_num=episode_num, task_id=f"large_task_{episode_num:03d}"
                )
                ...
                episode_data = EpisodeSummaryData(
                    ...
                    reward_progression=episode_rewards[
                        -10:
                    ],  # Last 10 rewards to save memory
        ...
        assert profiler.duration < 60.0, (
            f"Multiple large episodes took too long: {profiler.duration:.2f}s"
        )
        assert profiler.peak_memory_delta < 1000.0, (
            f"Memory usage too high: {profiler.peak_memory_delta:.2f}MB"
        )
    ...
    def test_memory_leak_detection(
        ...
        with patch(
            ...
            for episode_num in range(1, 11):  # 10 episodes
                performance_visualizer.start_episode(
                    episode_num=episode_num, task_id=f"leak_task_{episode_num}"
                )
        ...
        if len(memory_samples) > 5:
            ...
            assert correlation < 0.8, (
                f"Strong memory growth correlation detected: {correlation:.3f}"
            )
...
class TestConcurrentEpisodeProcessing:
    ...
    def create_concurrent_visualizer(
        self, temp_dir: Path, visualizer_id: int
    ) -> EnhancedVisualizer:
        ...
        async_logger = AsyncLogger(
            async_config, output_dir=temp_dir / f"logs_{visualizer_id}"
        )
        wandb_integration = WandbIntegration(wandb_config)
    ...
    def test_concurrent_visualizers(self, temp_dir: Path) -> None:
                ...
                with patch(
                    ...
                    for episode_num in range(1, episodes_per_visualizer + 1):
                        ...
                        for step in range(steps_per_episode):
                            try:
                                before_grid = (
                                    jnp.ones((5, 5), dtype=jnp.int32) * visualizer_id
                                )
        ...
        with ThreadPoolExecutor(max_workers=num_visualizers) as executor:
            futures = [
                executor.submit(run_visualizer, i) for i in range(num_visualizers)
            ]
        ...
        assert len(successful_results) == num_visualizers, (
            f"Only {len(successful_results)}/{num_visualizers} visualizers succeeded"
        )
        ...
        for result in successful_results:
            ...
            assert (
                result["steps_completed"] == steps_per_episode * episodes_per_visualizer
            )
            assert len(result["errors"]) == 0, (
                f"Visualizer {result['visualizer_id']} had errors: {result['errors']}"
            )
        ...
        assert profiler.duration < 30.0, (
            f"Concurrent processing took too long: {profiler.duration:.2f}s"
        )
        assert avg_duration < 15.0, (
            f"Average visualizer duration too long: {avg_duration:.2f}s"
        )
        ...
        for i in range(num_visualizers):
            ...
            assert concurrent_dir.exists(), (
                f"Output directory for visualizer {i} not found"
            )
    ...
    def test_thread_safety_stress_test(self, temp_dir: Path) -> None:
        ...
        try:
            ...
            with patch(
                ...
                for thread_id in range(num_threads):
                    thread = threading.Thread(
                        target=stress_visualize, args=(thread_id,)
                    )
            ...
            assert len(results) == num_threads, (
                f"Only {len(results)}/{num_threads} threads succeeded"
            )
            assert len(errors) == 0, f"Thread safety errors: {errors}"

            # Check that all steps were processed
            expected_steps = num_threads * 20
            assert visualizer.step_count == expected_steps, (
                f"Expected {expected_steps} steps, got {visualizer.step_count}"
            )
...
class TestStorageCleanupEfficiency:
    ...
    def test_cleanup_performance_large_dataset(self, temp_dir: Path) -> None:
        ...
        assert profiler.duration < 5.0, (
            f"Cleanup took too long: {profiler.duration:.2f}s"
        )
        ...
        assert len(remaining_dirs) <= episode_config.max_episodes_per_run, (
            "Too many episodes remaining"
        )

        # Check storage usage is within limits
        storage_usage = episode_manager.get_storage_usage_gb()
        assert storage_usage <= episode_config.max_storage_gb * 1.1, (
            f"Storage usage {storage_usage:.3f}GB exceeds limit"
        )
    ...
    def test_cleanup_different_policies_performance(self, temp_dir: Path) -> None:
        ...
        for policy in policies:
            ...
            policy_results[policy] = {
                ...
                "remaining_episodes": len(
                    [d for d in episode_manager.list_episodes() if d.exists()]
                ),
                "storage_usage": episode_manager.get_storage_usage_gb(),
            }
        ...
        for policy, results in policy_results.items():
            assert results["duration"] < 3.0, (
                f"{policy} cleanup took too long: {results['duration']:.2f}s"
            )
            assert results["remaining_episodes"] <= 20, (
                f"{policy} left too many episodes: {results['remaining_episodes']}"
            )
            assert results["storage_usage"] <= 0.1, (
                f"{policy} storage usage too high: {results['storage_usage']:.3f}GB"
            )

        # Both policies should be reasonably efficient
        duration_diff = abs(
            policy_results["oldest_first"]["duration"]
            - policy_results["size_based"]["duration"]
        )
        assert duration_diff < 2.0, (
            f"Large performance difference between policies: {duration_diff:.2f}s"
        )
...
class TestMemoryUsageAndLeakDetection:
    ...
    def test_memory_usage_scaling(self, temp_dir: Path) -> None:
        ...
        for episode_size in episode_sizes:
            ...
            episode_config = EpisodeConfig(
                base_output_dir=str(temp_dir / f"memory_test_{episode_size}")
            )
            ...
            episode_manager = EpisodeManager(episode_config)
            async_logger = AsyncLogger(
                async_config, output_dir=temp_dir / f"logs_{episode_size}"
            )
            wandb_integration = WandbIntegration(wandb_config)
            ...
            try:
                ...
                visualizer.start_episode(
                    episode_num=1, task_id=f"memory_task_{episode_size}"
                )
                ...
                with patch(
                    ...
                    for step in range(episode_size):
                        grid_size = min(
                            10, step // 10 + 3
                        )  # Gradually increase grid size
                        before_grid = (
                            jnp.ones((grid_size, grid_size), dtype=jnp.int32) * step
                        )
        ...
        for i in range(1, len(episode_sizes_sorted)):
            ...
            memory_ratio = memory_deltas[i] / max(
                memory_deltas[i - 1], 1.0
            )  # Avoid division by zero

            # Memory growth should be sub-linear (better than O(n))
            assert memory_ratio < size_ratio * 1.5, (
                f"Memory scaling too aggressive: {memory_ratio:.2f}x for {size_ratio:.2f}x size increase"
            )

        # Absolute memory usage should be reasonable
        max_memory_delta = max(memory_deltas)
        assert max_memory_delta < 200.0, (
            f"Maximum memory usage too high: {max_memory_delta:.2f}MB"
        )
    ...
    def test_async_logger_memory_management(self, temp_dir: Path) -> None:
        ...
        try:
            ...
            assert memory_growth < 100.0, (
                f"Final memory growth too high: {memory_growth:.2f}MB"
            )
            assert peak_memory_growth < 200.0, (
                f"Peak memory growth too high: {peak_memory_growth:.2f}MB"
            )

            # Check that queue doesn't grow unbounded
            queue_size = async_logger.queue.qsize()
            assert queue_size < async_config.queue_size * 0.8, (
                f"Queue size too high: {queue_size}"
            )
    ...
    def test_garbage_collection_effectiveness(self, temp_dir: Path) -> None:
        ...
        try:
            ...
            for cycle in range(10):
                ...
                visualizer.start_episode(
                    episode_num=cycle + 1, task_id=f"gc_task_{cycle}"
                )
            ...
            assert avg_cycle_growth < 20.0, (
                f"Average cycle memory growth too high: {avg_cycle_growth:.2f}MB"
            )

            # Total memory growth should be bounded
            total_growth = (
                memory_samples[-1]["post_cycle"] - memory_samples[0]["pre_cycle"]
            )
            ...
            decreasing_cycles = sum(
                1
                for i in range(1, len(cycle_growths))
                if cycle_growths[i] < cycle_growths[i - 1]
            )
            assert decreasing_cycles >= 2, (
                "Memory should decrease in some cycles due to GC"
            )
