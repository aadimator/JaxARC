...
def mock_structured_logger(temp_dir):
    ...
    config = LoggingConfig(output_dir=str(temp_dir / "logs"), include_full_states=False)
...
class TestEpisodeReplaySystem:
    ...
    def test_load_episode_not_found(self, replay_system):
        ...
        with patch.object(
            replay_system.structured_logger, "load_episode", return_value=None
        ):
    ...
    def test_load_episode_with_cache(self, replay_system, sample_episode):
        ...
        with patch.object(
            replay_system.structured_logger, "load_episode", return_value=sample_episode
        ):
            # First load
    ...
    def test_validate_episode_integrity_invalid_steps(
        self, replay_system, sample_episode
    ):
    ...
    def test_validate_episode_integrity_invalid_timestamps(
        self, replay_system, sample_episode
    ):
    ...
    def test_replay_episode_success(self, replay_system, sample_episode, temp_dir):
        ...
        with patch.object(replay_system, "load_episode", return_value=sample_episode):
            result = replay_system.replay_episode(
                1, validate=False, regenerate_visualizations=False
            )
            ...
            summary_path = (
                temp_dir / "replay" / "episode_0001_replay" / "replay_summary.json"
            )
    ...
    def test_list_available_episodes(self, replay_system):
        ...
        with patch.object(
            replay_system.structured_logger, "list_episodes", return_value=[1, 2, 3]
        ):
    ...
    def test_get_episode_summaries(self, replay_system):
        ...
        with patch.object(
            replay_system.structured_logger, "get_episode_summary"
        ) as mock_get:
    ...
    def test_find_episodes_by_criteria(self, replay_system):
        ...
        with patch.object(
            replay_system, "get_episode_summaries", return_value=mock_summaries
        ):
            ...
            episodes = replay_system.find_episodes_by_criteria(
                min_reward=0.2, task_id="task1"
            )
            assert episodes == [1]
...
class TestEpisodeAnalysisTools:
    ...
    def test_analyze_performance_metrics_empty(self, analysis_tools):
        ...
        with patch.object(
            analysis_tools.replay_system, "get_episode_summaries", return_value=[]
        ):
    ...
    def test_compare_episodes(self, analysis_tools):
        ...
        with patch.object(
            analysis_tools.replay_system.structured_logger, "get_episode_summary"
        ) as mock_get:
            mock_get.side_effect = lambda x: mock_summaries[x - 1] if x <= 2 else None

            comparison = analysis_tools.compare_episodes(
                [1, 2], metrics=["reward", "similarity"]
            )
    ...
    def test_generate_step_by_step_analysis(self, analysis_tools, sample_episode):
        ...
        with patch.object(
            analysis_tools.replay_system, "load_episode", return_value=sample_episode
        ):
    ...
    def test_generate_step_by_step_analysis_not_found(self, analysis_tools):
        ...
        with patch.object(
            analysis_tools.replay_system, "load_episode", return_value=None
        ):
    ...
    def test_export_analysis_report(self, analysis_tools, temp_dir):
        ...
        with patch.object(
            analysis_tools.replay_system, "list_available_episodes", return_value=[1]
        ):
            with patch.object(
                analysis_tools.replay_system,
                "get_episode_summaries",
