...
class TestEquinoxStateMigration:
    ...
    def test_execute_grid_operation_equinox(self, demo_state):
        ...
        demo_state = eqx.tree_at(
            lambda s: s.selected,
            demo_state,
            jnp.zeros_like(demo_state.working_grid, dtype=jnp.bool_).at[0:2, 0:2].set(True)
        )
    ...
    def test_performance_comparison(self, demo_state):
        ...
        assert equinox_time < replace_time * 5.0, f"Equinox too slow: {equinox_time} vs {replace_time}"
        
        # Log the performance comparison for information
        print(f"Performance comparison: eqx.tree_at={equinox_time:.4f}s, replace={replace_time:.4f}s")
