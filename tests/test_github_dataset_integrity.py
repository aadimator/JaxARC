...
class TestGitHubDatasetIntegrity:
                    ...
                    "train": [
                        {
                            ...
                            "output": [
                                [(j + 1) % 10 for j in range(30)] for _ in range(30)
                            ],
                        }
                    ],
                    "test": [
                        {
                            "input": [
                                [(j + 2) % 10 for j in range(30)] for _ in range(30)
                            ],
                            "output": [
                                [(j + 3) % 10 for j in range(30)] for _ in range(30)
                            ],
                        }
                    ],
                }
    ...
    def corrupted_dataset(self):
        ...
        with malformed_json.open("w", encoding="utf-8") as f:
            f.write(
                '{"train": [{"input": [[1, 2]], "output": [[2, 1]]}], "test": [{"input": [[3, 4]]}'
    ...
    def test_all_expected_tasks_loaded(self, large_github_dataset):
        ...
        assert len(task_ids) == expected_count, (
            f"Expected {expected_count} tasks, got {len(task_ids)}"
        )
    ...
    def test_task_data_structure_matches_jaxarctask_format(self, large_github_dataset):
        ...
        for task_id in test_tasks:
            ...
            assert task.test_input_masks.shape == (
                max_test_pairs,
                max_height,
                max_width,
            )
    ...
    def test_parser_performance_with_large_datasets(self, large_github_dataset):
        ...
        assert init_time < 30.0, (
            f"Parser initialization took too long: {init_time:.2f}s"
        )
        ...
        assert random_access_time < 10.0, (
            f"Random access took too long: {random_access_time:.2f}s"
        )

        # Test specific task access
        start_time = time.time()

        for i in range(min(50, len(task_ids))):  # Access first 50 tasks by ID
        ...
        assert specific_access_time < 5.0, (
            f"Specific access took too long: {specific_access_time:.2f}s"
        )
            ...
            if task_type == "large_grid":
                ...
                for i in range(30):
                    for j in range(30):
                        ...
                        assert actual_value == expected_value, (
                            f"Data corruption at [{i},{j}]: expected {expected_value}, got {actual_value}"
                        )
                ...
                for i in range(30):
                    for j in range(30):
                        ...
                        assert actual_value == expected_value, (
                            f"Output corruption at [{i},{j}]: expected {expected_value}, got {actual_value}"
                        )

            elif task_type == "multiple_train":
                # Verify multiple training pairs
                assert task.num_train_pairs == 3, (
                    f"Expected 3 training pairs, got {task.num_train_pairs}"
                )
                ...
                for pair_idx in range(task.num_train_pairs):
                    ...
                    assert jnp.sum(input_mask) > 0, (
                        f"Training pair {pair_idx} has empty mask"
                    )

                    # Verify data values are within valid range
                    valid_input_values = input_grid[input_mask]
                    assert jnp.all(valid_input_values >= 0), (
                        f"Training pair {pair_idx} has negative input values"
                    )
                    assert jnp.all(valid_input_values <= 9), (
                        f"Training pair {pair_idx} has input values > 9"
                    )

                    valid_output_values = output_grid[
                        input_mask
                    ]  # Use same mask for output
                    assert jnp.all(valid_output_values >= 0), (
                        f"Training pair {pair_idx} has negative output values"
                    )
                    assert jnp.all(valid_output_values <= 9), (
                        f"Training pair {pair_idx} has output values > 9"
                    )
                ...
                assert task.num_test_pairs >= 1, (
                    f"Expected at least 1 test pair, got {task.num_test_pairs}"
                )
                ...
                for pair_idx in range(task.num_test_pairs):
                    ...
                    # Verify mask covers valid region
                    assert jnp.sum(input_mask) > 0, (
                        f"Test pair {pair_idx} has empty mask"
                    )

                    # Verify data values are within valid range
                    valid_input_values = input_grid[input_mask]
                    assert jnp.all(valid_input_values >= 0), (
                        f"Test pair {pair_idx} has negative input values"
                    )
                    assert jnp.all(valid_input_values <= 9), (
                        f"Test pair {pair_idx} has input values > 9"
                    )

            elif task_type == "standard":
                # Verify standard task structure
                assert task.num_train_pairs == 1, (
                    f"Expected 1 training pair, got {task.num_train_pairs}"
                )
                assert task.num_test_pairs == 1, (
                    f"Expected 1 test pair, got {task.num_test_pairs}"
                )
                ...
                if len(valid_positions[0]) >= 2:
                    ...
                    actual_input = [
                        int(input_grid[valid_positions[0][0], valid_positions[1][0]]),
                        int(input_grid[valid_positions[0][1], valid_positions[1][1]]),
                    ]
                    actual_output = [
                        int(output_grid[valid_positions[0][0], valid_positions[1][0]]),
                        int(output_grid[valid_positions[0][1], valid_positions[1][1]]),
                    ]

                    assert actual_input == expected_input, (
                        f"Input data corruption: expected {expected_input}, got {actual_input}"
                    )
                    assert actual_output == expected_output, (
                        f"Output data corruption: expected {expected_output}, got {actual_output}"
                    )
    ...
    def test_memory_usage_with_large_datasets(self, large_github_dataset):
        ...
        assert memory_growth < 100, (
            f"Memory growth during access too high: {memory_growth:.1f}MB"
        )
    ...
    def test_concurrent_access_safety(self, large_github_dataset):
        ...
        for i in range(num_threads):
            thread = threading.Thread(
                target=worker_thread, args=(i, num_accesses_per_thread)
            )
        ...
        for thread_id, thread_results in results:
            assert len(thread_results) == num_accesses_per_thread, (
                f"Thread {thread_id} incomplete"
            )
            # All results should be positive (valid num_train_pairs)
            assert all(r > 0 for r in thread_results), (
                f"Thread {thread_id} has invalid results"
            )
    ...
    def test_data_consistency_across_multiple_loads(self, large_github_dataset):
        ...
        assert all(task_ids == task_ids_sets[0] for task_ids in task_ids_sets), (
            "Task IDs differ between parser instances"
        )
        ...
        for task_id in test_task_ids:
            ...
            for i, task in enumerate(tasks[1:], 1):
                ...
                for field in array_fields:
                    ...
                    assert jnp.array_equal(ref_array, task_array), (
                        f"Task {task_id}, parser {i}, field {field}: arrays differ"
                    )

                # Compare scalar fields
                assert task.num_train_pairs == reference_task.num_train_pairs, (
                    f"Task {task_id}, parser {i}: num_train_pairs differ"
                )
                assert task.num_test_pairs == reference_task.num_test_pairs, (
                    f"Task {task_id}, parser {i}: num_test_pairs differ"
                )
                assert jnp.array_equal(task.task_index, reference_task.task_index), (
                    f"Task {task_id}, parser {i}: task_index differ"
                )
