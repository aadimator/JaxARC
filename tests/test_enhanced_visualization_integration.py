...
class TestVisualizationPipelineIntegration:
    ...
    def test_complete_episode_workflow(self, integrated_visualizer: EnhancedVisualizer, temp_dir: Path) -> None:
        ...
        integrated_visualizer.start_episode(episode_num=1, task_id="integration_task_001")
        
        # Mock visualization functions
        with patch('jaxarc.utils.visualization.enhanced_visualizer.draw_rl_step_svg') as mock_step_viz, \
             patch('jaxarc.utils.visualization.enhanced_visualizer.create_episode_summary_svg') as mock_summary_viz, \
             patch('jaxarc.utils.visualization.enhanced_visualizer.convert_svg_to_png') as mock_png_convert:
    ...
    def test_multiple_episodes_workflow(self, integrated_visualizer: EnhancedVisualizer, temp_dir: Path) -> None:
        ...
        with patch('jaxarc.utils.visualization.enhanced_visualizer.draw_rl_step_svg') as mock_step_viz, \
            ...
            for episode_num in range(1, 4):
                integrated_visualizer.start_episode(episode_num=episode_num, task_id=f"task_{episode_num:03d}")
        ...
        with patch('jaxarc.utils.visualization.enhanced_visualizer.create_comparison_svg') as mock_comparison:
    ...
    def test_jax_performance_impact_measurement(self, integrated_visualizer: EnhancedVisualizer) -> None:
        ...
        assert performance_impact < 0.1, f"Performance impact too high: {performance_impact:.2%}"
    ...
    def test_error_handling_and_recovery(self, integrated_visualizer: EnhancedVisualizer, temp_dir: Path) -> None:
        ...
        with patch('jaxarc.utils.visualization.enhanced_visualizer.draw_rl_step_svg', side_effect=failing_visualization):
        ...
        try:
            ...
            def run_episode(visualizer_id: int, visualizer: EnhancedVisualizer):
                try:
                    ...
                    visualizer.start_episode(episode_num=1, task_id=f"concurrent_task_{visualizer_id}")
            ...
            # Check results
            assert len(results) == 3, f"Expected 3 successes, got {len(results)}: {results}"
    ...
    def test_storage_cleanup_integration(self, integrated_visualizer: EnhancedVisualizer, temp_dir: Path) -> None:
        """Test storage cleanup integration with episode management."""
        # Configure small storage limits
        integrated_visualizer.episode_manager.config = integrated_visualizer.episode_manager.config.replace(
        ...
        with patch('jaxarc.utils.visualization.enhanced_visualizer.draw_rl_step_svg') as mock_viz:
            mock_viz.return_value = "<svg>" + "x" * 1000 + "</svg>"  # Large visualization
            
            episode_dirs = []
            
            # Create episodes that exceed storage limits
            for episode_num in range(5):  # More than max_episodes_per_run
                integrated_visualizer.start_episode(episode_num=episode_num + 1, task_id=f"cleanup_task_{episode_num}")
                episode_dirs.append(integrated_visualizer.episode_manager.current_episode_dir)
                
                # Create large visualizations
                for step in range(3):
                    integrated_visualizer.visualize_step(
                        before_grid=jnp.ones((10, 10), dtype=jnp.int32),
                        action={"type": "cleanup_test", "episode": episode_num, "step": step},
                ...
                (integrated_visualizer.episode_manager.current_episode_dir / "large_file.txt").write_text(large_content)
        ...
        assert storage_usage <= 0.01, f"Storage usage {storage_usage} exceeds expected limit"
...
class TestConfigurationIntegration:
    ...
    def test_environment_specific_configs(self, temp_dir: Path) -> None:
        ...
        assert dev_config["visualization"]["debug_level"] == "verbose"  # More debugging in dev
        assert prod_config["visualization"]["debug_level"] == "minimal"  # Less overhead in prod
        
        assert dev_config["episode_management"]["cleanup_policy"] == "manual"  # Manual cleanup in dev
        assert prod_config["episode_management"]["cleanup_policy"] == "size_based"  # Automatic in prod
