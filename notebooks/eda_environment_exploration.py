...
console.print(f"[green]Selected random task: {parsed_task.task_index}[/green]")

console.print(
    Panel(
        f"[bold cyan]Task Structure[/bold cyan]\n\n"
        f"Task ID: {parsed_task.task_index}\n"
...
visualize_parsed_task_data_rich(parsed_task)
...
try:
    ...
    new_obs, new_state, rewards, dones, infos = env.step_env(
        step_key, initial_state, actions
    )
...
action_sequence = [
    (
        "Fill region with blue",
        create_action((2, 2, 6, 6), 1, env.max_grid_size),
    ),  # Fill with blue
    (
        "Copy to clipboard",
        create_action((2, 2, 6, 6), 28, env.max_grid_size),
    ),  # Copy to clipboard
    (
        "Paste elsewhere",
        create_action((10, 10, 14, 14), 29, env.max_grid_size),
    ),  # Paste from clipboard
    (
        "Fill with red",
        create_action((15, 15, 20, 20), 2, env.max_grid_size),
    ),  # Fill with red
]
...
for i, (description, action) in enumerate(action_sequence):
    try:
        ...
        obs, new_state, rewards, dones, infos = env.step_env(
            step_key, current_state, actions
        )
        ...
        console.print(
            f"  Operation: {step_result['operation']}, Reward: {step_result['reward']:.3f}, Similarity: {step_result['similarity']:.3f}"
        )
...
if parsed_task.num_train_pairs > 1:
    ...
    for pair_idx in range(min(3, parsed_task.num_train_pairs)):
        ...
        console.print(
            f"Colors: {pair_input_colors.tolist()} → {pair_output_colors.tolist()}"
        )
