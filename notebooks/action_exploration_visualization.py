...
from jaxarc.types import ARCLEAction, ARCLEOperationType, JaxArcTask
...
def load_and_explore_dataset(config):
    ...
    try:
        ...
        console.print("[yellow]This is expected if you haven't downloaded the ARC-AGI dataset yet.[/yellow]")
        console.print("[yellow]You can still explore the notebook structure and operation definitions.[/yellow]")
        return None, []
...
def display_operation_definitions():
    ...
    # Create a table
    table = Table(title="Available Operations (0-34)", show_header=True, header_style="bold magenta")
...
def create_selection_mask(grid_shape, working_grid_mask=None, selection_type="single_pixel", x=0, y=0, width=1, height=1):
...
def visualize_state_comparison(state_before, state_after, operation_name):
    ...
    try:
        ...
        before_working_grid = extract_working_grid(state_before.working_grid, state_before.working_grid_mask)
        log_grid_to_console(before_working_grid, title="Before (Working Area)", show_numbers=False)
    except Exception as e:
    ...
    try:
        ...
        # Extract only the working grid area for visualization
        after_working_grid = extract_working_grid(state_after.working_grid, state_after.working_grid_mask)
        log_grid_to_console(after_working_grid, title="After (Working Area)", show_numbers=False)
    except Exception as e:
        console.print(f"[red]Error visualizing after grid: {e}[/red]")

    # Show similarity score
    console.print(f"[bold]Similarity Score: {float(state_after.similarity_score):.4f}[/bold]")
    ...
    if jnp.sum(state_before.selected) > 0:
        try:
            ...
            selection_working_area = extract_working_grid(state_before.selected.astype(jnp.int32), state_before.working_grid_mask)
            log_grid_to_console(selection_working_area, title="Selection (Working Area)", show_numbers=False)
        except Exception as e:
...
def test_fill_operations(initial_state, grid_shape):
    ...
    console.print(Panel("[bold]Testing Fill Operations (0-9)[/bold]", border_style="blue"))

    # Create a small rectangular selection within working grid bounds
    selection = create_selection_mask(grid_shape, initial_state.working_grid_mask, "rectangle", x=1, y=1, width=2, height=2)
...
def test_flood_fill_operations(initial_state, grid_shape):
    ...
    console.print(Panel("[bold]Testing Flood Fill Operations (10-19)[/bold]", border_style="green"))

    # Create a single pixel selection for flood fill within working grid
    selection = create_selection_mask(grid_shape, initial_state.working_grid_mask, "single_pixel", x=2, y=2)
...
def test_movement_operations(initial_state, grid_shape):
    ...
    console.print(Panel("[bold]Testing Movement Operations (20-23)[/bold]", border_style="yellow"))
...
def test_rotation_operations(initial_state, grid_shape):
    ...
    console.print(Panel("[bold]Testing Rotation Operations (24-25)[/bold]", border_style="magenta"))

    # Create an L-shaped selection within working grid bounds
    min_row, max_row, min_col, max_col = get_working_grid_bounds(initial_state.working_grid_mask)
    ...
    if max_row - min_row >= 2 and max_col - min_col >= 2:
        selection = selection.at[min_row:min_row+3, min_col].set(True)  # Vertical line
        selection = selection.at[min_row+2, min_col:min_col+3].set(True)  # Horizontal line
        ...
        selection = create_selection_mask(grid_shape, initial_state.working_grid_mask, "rectangle", x=0, y=0, width=2, height=2)
...
def test_clipboard_operations(initial_state, grid_shape):
    ...
    console.print(Panel("[bold]Testing Clipboard Operations (28-30)[/bold]", border_style="cyan"))

    # Create selection for copying within working grid
    copy_selection = create_selection_mask(grid_shape, initial_state.working_grid_mask, "rectangle", x=0, y=0, width=2, height=2)
    ...
    paste_selection = create_selection_mask(grid_shape, initial_state.working_grid_mask, "rectangle", x=paste_x, y=paste_y, width=2, height=2)
    ...
    state_after_paste = execute_grid_operation(state_with_paste_selection, action.operation)



    visualize_state_comparison(
        state_with_paste_selection,
        state_after_paste,
...
def create_test_environment_state(parsed_task=None):
    ...
    if parsed_task is None:
        ...
        test_grid = test_grid.at[0:working_size, 0:working_size].set(0)  # Set working area to background
        ...
        working_grid_mask = working_grid_mask.at[0:working_size, 0:working_size].set(True)

        target_grid = jnp.full(grid_shape, -1, dtype=jnp.int32)  # Fill with -1 (padding)
        target_grid = target_grid.at[0:working_size, 0:working_size].set(0)  # Set working area to background
...
console.print(Panel("[bold]Test Environment State Created[/bold]", border_style="green"))
console.print("[bold]Initial Working Grid:[/bold]")
try:
    # Extract only the working grid area for visualization
    initial_working_grid = extract_working_grid(test_state.working_grid, test_state.working_grid_mask)
    log_grid_to_console(initial_working_grid, title="Initial Working Grid", show_numbers=False)
except Exception as e:
...
def minimal_grid_test():
    ...
    console.print(Panel("[bold]Running Minimal Grid Test[/bold]", border_style="yellow"))
    ...
    try:
        ...
        selection = jnp.array([[True, False, False], [False, True, False], [False, False, True]], dtype=jnp.bool_)
    ...
    try:
        ...
        console.print(f"Minimal state working_grid shape: {minimal_state.working_grid.shape}")
        ...
        result_working_grid = extract_working_grid(new_state.working_grid, new_state.working_grid_mask)
        log_grid_to_console(result_working_grid, title="After Operation (Working Area)", show_numbers=False)
...
def test_grid_operations(initial_state, grid_shape):
    ...
    console.print(Panel("[bold]Testing Grid Operations (31-33)[/bold]", border_style="red"))

    # Test clear operation - create selection within working grid bounds
    selection = create_selection_mask(grid_shape, initial_state.working_grid_mask, "rectangle", x=1, y=1, width=3, height=3)
    ...
    state_with_empty_selection = state_after_clear.replace(selected=jnp.zeros(grid_shape, dtype=jnp.bool_))
    state_after_copy_input = execute_grid_operation(state_with_empty_selection, action.operation)

    visualize_state_comparison(
        state_after_clear,
        state_after_copy_input,
        "Copy Input Grid"
    )
    ...
    resize_selection = create_selection_mask(grid_shape, initial_state.working_grid_mask, "rectangle", x=1, y=1, width=resize_width, height=resize_height)
    action = create_action(33, resize_selection)  # Resize

    # Set selection in state before executing operation
    state_with_resize_selection = state_after_copy_input.replace(selected=resize_selection)
    state_after_resize = execute_grid_operation(state_with_resize_selection, action.operation)

    visualize_state_comparison(
        state_with_resize_selection,
        state_after_resize,
        "Resize Working Grid"
    )
...
def test_submit_operation(initial_state, grid_shape):
    ...
    console.print(Panel("[bold]Testing Submit Operation (34)[/bold]", border_style="green"))
    ...
    console.print(f"[bold]Before Submit - Episode Done: {initial_state.episode_done}[/bold]")
    console.print(f"[bold]After Submit - Episode Done: {state_after_submit.episode_done}[/bold]")
...
def test_random_actions(initial_state, grid_shape, num_actions=5):
    ...
    console.print(Panel(f"[bold]Testing {num_actions} Random Actions[/bold]", border_style="blue"))
    ...
    for i in range(num_actions):
        ...
        selection_type = jr.choice(sel_key, jnp.array(["single_pixel", "rectangle", "none"]))

        if selection_type == 0:  # single_pixel
            x = jr.randint(action_key, (), 0, grid_shape[1])
            y = jr.randint(action_key, (), 0, grid_shape[0])
            selection = create_selection_mask(grid_shape, current_state.working_grid_mask, "single_pixel", x=int(x), y=int(y))
            ...
            selection = create_selection_mask(grid_shape, current_state.working_grid_mask, "rectangle", x=int(x), y=int(y), width=int(width), height=int(height))
        else:  # none
            selection = create_selection_mask(grid_shape, current_state.working_grid_mask, "none")
        ...
        # Get operation name
        operation_names = {
            0: "Fill Color 0", 1: "Fill Color 1", 2: "Fill Color 2", 3: "Fill Color 3",
            4: "Fill Color 4", 5: "Fill Color 5", 6: "Fill Color 6", 7: "Fill Color 7",
            8: "Fill Color 8", 9: "Fill Color 9", 10: "Flood Fill 0", 11: "Flood Fill 1",
            12: "Flood Fill 2", 13: "Flood Fill 3", 14: "Flood Fill 4", 15: "Flood Fill 5",
            16: "Flood Fill 6", 17: "Flood Fill 7", 18: "Flood Fill 8", 19: "Flood Fill 9",
            20: "Move Up", 21: "Move Down", 22: "Move Left", 23: "Move Right",
            24: "Rotate 90° CW", 25: "Rotate 90° CCW", 26: "Flip Horizontal", 27: "Flip Vertical",
            28: "Copy", 29: "Paste", 30: "Cut", 31: "Clear", 32: "Copy Input", 33: "Resize"
        }

        op_name = operation_names.get(int(operation_id), f"Unknown Op {operation_id}")

        console.print(f"\n[bold blue]Random Action {i+1}: {op_name}[/bold blue]")
        visualize_state_comparison(current_state.replace(selected=selection), new_state, op_name)
...
def validate_implementation():
    ...
    try:
        selection = create_selection_mask(grid_shape, test_state.working_grid_mask, "single_pixel", x=2, y=2)
        action = create_action(1, selection)
        validation_results.append(("Action Creation", "✓ PASS", "Actions created successfully"))
    except Exception as e:
        validation_results.append(("Action Creation", "✗ FAIL", f"Error: {e}"))

    # Test 2: Grid operations
    try:
        selection = create_selection_mask(grid_shape, test_state.working_grid_mask, "rectangle", x=1, y=1, width=2, height=2)
        ...
        new_state = execute_grid_operation(state_with_selection, action.operation)
        validation_results.append(("Grid Operations", "✓ PASS", "Grid operations execute without errors"))
    except Exception as e:
    ...
    try:
        ...
        validation_results.append(("State Consistency", "✓ PASS", "State structure maintained"))
    except Exception as e:
    ...
    try:
        ...
        validation_results.append(("JAX Compatibility", "✓ PASS", "Functions are JIT compatible"))
    except Exception as e:
    ...
    if passed == total:
        console.print("[green]✓ All validation tests passed! Implementation is working correctly.[/green]")
    else:
        console.print("[red]✗ Some validation tests failed. Please review implementation.[/red]")
