...
point_config = ArcEnvConfig.from_hydra(point_cfg, parser=parser)
...
def create_action_demonstration_figure(env, initial_state, action_format, demo_actions):
    ...
    create_grid_matplotlib(initial_state.target_grid, title="Target Grid", ax=ax_target)
    ...
    for i, (action, description) in enumerate(demo_actions):
        ...
        try:
            # Execute action
            next_state, obs, reward, info = env.step(action)
