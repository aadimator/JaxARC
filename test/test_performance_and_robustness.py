...
from jaxarc.types import Grid, JaxArcTask, TaskPair, TimeStep, StepType
...
class TestPerformanceCharacteristics:
    ...
    def test_grid_creation_performance(self):
        ...
        for size in sizes:
            ...
            assert creation_time < 1.0, f"Grid creation took {creation_time:.3f}s for size {size}x{size}"
    ...
    def test_task_access_performance(self):
        ...
        assert access_time < 1.0, f"Pair access took {access_time:.3f}s for {max_pairs} pairs"
    ...
    def test_batch_processing_performance(self):
        ...
        assert batch_time < 2.0, f"Batch processing took {batch_time:.3f}s for {batch_size} grids"
        assert len(grids) == batch_size
    ...
    def test_stress_test_performance(self):
        ...
        assert late_avg < early_avg * 2, f"Performance degraded: early={early_avg:.4f}s, late={late_avg:.4f}s"
    ...
    def test_task_concurrent_access(self):
        ...
        def access_task():
            try:
                ...
                return (summary, train_pair.input_grid.shape, test_pair.input_grid.shape)
            except Exception as e:
                errors.append(e)
                return None
...
class TestErrorRecovery:
    ...
    def test_partial_failure_handling(self):
        ...
        expected_successful = batch_size - (batch_size + 2) // 3  # More accurate calculation
    ...
    def test_state_consistency_after_errors(self):
        ...
        for i in range(5):
            try:
                ...
                invalid_config = JaxArcConfig(
                    environment={"max_grid_size": -1}
                )
