...
class TestPaddingFunctions:
    ...
    def test_pad_grid_to_size_exceeds_target(self):
        ...
        with pytest.raises(ValueError, match="Grid dimensions.*exceed target dimensions"):
...
class TestRectangleExtraction:
    ...
    def test_extract_object_rectangle_basic(self):
        ...
        expected_masked = jnp.where(
            (jnp.arange(3)[:, None] >= 0) & (jnp.arange(3)[:, None] <= 1) &
            (jnp.arange(4)[None, :] >= 0) & (jnp.arange(4)[None, :] <= 2),
            expected_masked, 0
        )
