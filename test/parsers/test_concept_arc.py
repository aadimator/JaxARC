...
class TestConceptArcParser:
    ...
    @pytest.fixture
    def mock_parser_with_data(self, valid_config: DatasetConfig, temp_concept_arc_dataset: Path) -> ConceptArcParser:
    ...
    def test_get_tasks_in_concept_invalid(self, mock_parser_with_data: ConceptArcParser):
        """Test getting tasks from non-existent concept group."""
        with pytest.raises(ValueError, match="not found"):
            mock_parser_with_data.get_tasks_in_concept("NonExistentConcept")

    def test_load_task_file_success(self, mock_parser_with_data: ConceptArcParser, temp_concept_arc_dataset: Path):
    ...
    def test_preprocess_task_data_with_tuple(self, mock_parser_with_data: ConceptArcParser, sample_task_data: dict):
        """Test preprocessing with (task_id, task_content) tuple."""
        key = jax.random.PRNGKey(42)
        result = mock_parser_with_data.preprocess_task_data(("test_task", sample_task_data), key)
        
        assert isinstance(result, JaxArcTask)
        assert result.num_train_pairs == 2
        assert result.num_test_pairs == 1

    def test_preprocess_task_data_direct_content(self, mock_parser_with_data: ConceptArcParser, sample_task_data: dict):
    ...
    def test_get_random_task_from_concept_success(self, mock_parser_with_data: ConceptArcParser):
    ...
    def test_get_random_task_from_concept_invalid(self, mock_parser_with_data: ConceptArcParser):
    ...
    def test_get_random_task_from_concept_no_tasks(self, mock_parser_with_data: ConceptArcParser):
    ...
    def test_concept_based_sampling_consistency(self, mock_parser_with_data: ConceptArcParser):
    ...
    def test_error_handling_corrupted_json(self, temp_concept_arc_dataset: Path, valid_config: DatasetConfig):
    ...
    def test_deterministic_preprocessing(self, mock_parser_with_data: ConceptArcParser):
        ...
        result1 = mock_parser_with_data.preprocess_task_data(("test_task", task_data), key1)
        result2 = mock_parser_with_data.preprocess_task_data(("test_task", task_data), key2)
